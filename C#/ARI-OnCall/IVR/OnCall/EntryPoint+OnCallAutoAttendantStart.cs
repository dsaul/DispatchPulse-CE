...
namespace ARI.IVR.OnCall
{
	public partial class EntryPoint : AGIScriptPlus
	{
		...
		protected async Task OnCallAutoAttendantStart(AGIRequest request, AGIChannel channel, LeaveMessageRequestData requestData) {

			requestData.MessageLeftAtISO8601 = DateTime.UtcNow.ToString("o", Culture.DevelopmentCulture);

			if (null == requestData.DPDID)
				await ThrowError(request, "ds42", "null == data.DPDID");
			if (null == requestData.DPDB)
				await ThrowError(request, "4244", "null == data.DPDB");
			if (null == requestData.DPDID.AssignToID) {
				await PlayTTS("This phone number doesn't have a on call responder attendant assigned to it. Please check the phone number page, or contact support.", "", Engine.Neural, VoiceId.Brian);
				throw new PerformHangupException();
			}

			var resAA = OnCallAutoAttendants.ForId(requestData.DPDB, requestData.DPDID.AssignToID.Value);
			if (0 == resAA.Count)
				await ThrowError(request, "3255", "0 == resAA.Count");
			...
			if (null == requestData.AutoAttendant.RecordingsIntroType)
				await ThrowError(request, "1566", "null == data.AutoAttendant.RecordingsIntroType");
			if (null == requestData.AutoAttendant.RecordingsIntroText)
				await ThrowError(request, "1567", "null == data.AutoAttendant.RecordingsIntroText");
			...
			while ((--i) >= 0) {
				...
				switch (requestData.AutoAttendant.RecordingsIntroType) {
					case OnCallAutoAttendants.RecordingTypeE.Polly:
						if (key == '\0') {
							key = await PlayTTS(requestData.AutoAttendant.RecordingsIntroText, kEscapeAllKeys, Engine.Neural, VoiceId.Brian);
						}
						break;
					case OnCallAutoAttendants.RecordingTypeE.Recording:
						if (key == '\0') {
							...
							key = await PlayRecording(requestData.DPDB, recordingId.Value, kEscapeAllKeys);
				...
				switch (key) {
					case '1':
						...
						await OnCallAutoAttendantConfirmedLeavingMessage(request, channel, requestData);
						throw new PerformHangupException();
					default:
						if (i != 0) {
							await PlayTTS("That isn't a valid option, please try again.", kEscapeAllKeys, Engine.Neural, VoiceId.Brian);
						} else {
							await PlayTTS("That isn't a valid option.", kEscapeAllKeys, Engine.Neural, VoiceId.Brian);
						}
			...
			await PlayTTS("Unfortunately we weren't able to get a response, please try again later.", kEscapeAllKeys, Engine.Neural, VoiceId.Brian);
			throw new PerformHangupException();
		}
