...
namespace JobRunnerCompaniesReport
{
	class Program
	{
		static void Main() {
			...
			while (true) {
				...
				JobRunnerJob? job = JobRunnerJob.ClaimJobIfAvailable(jobsDB, JobRunnerJob.kJobTypeValueRunReportCompanies);
				if (job != null) {
					RunReportCompanies(jobsDB, job);
				}
		...
		public static void RunReportCompanies(
			NpgsqlConnection jobsDB,
			JobRunnerJob job
			) {

			Log.Debug($"RunReportCompanies(db, {job.Id})");
			...
			do {
				...
				if (null != job.RunOnAllCompanies && job.RunOnAllCompanies.Value) {
					var res = Companies.All(dpDB);
					list.AddRange(res.Values);
				} else {
					var res = Companies.ForIds(dpDB, job.CompanyIds);
					list.AddRange(res.Values);
				}

				if (list.Count == 0) {
					error = "No companies to generate a report on.";
					break;
				}
				...
				latex = LaTeXCompanies.Generate(billingDB, dpDB, true, true, list);
