...
namespace ARI.IVR.CompanyAccess
{
	...
	{
		protected void SaveAndEndThisWorkTimer(
			AGIRequest request,
			AGIChannel channel,
			RequestData data,
			Labour labour
			...
			var payload = new PerformSaveAndEndThisWorkTimerParams(
			...
			PerformSaveAndEndThisWorkTimerResponse response =
				Program.SignalRConnection.InvokeAsync<PerformSaveAndEndThisWorkTimerResponse>(
					"PerformSaveAndEndThisWorkTimer", payload).Result;
			...
			PlayTTS("Your work timer has been completed.", "", Engine.Neural, VoiceId.Brian);
