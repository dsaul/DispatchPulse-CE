...
namespace JobRunnerOnCallResponder30Days
{
	class Program
	{
		static void Main() {
		...
		public static void RunReportOnCallResponder30Day(
			NpgsqlConnection jobsDB,
			JobRunnerJob job
			) {

			Log.Debug($"RunReportOnCallResponder30Day(db, {job.Id})");
			...
			do {
				...
				latex = LaTeXOnCallResponder30Days.Generate(billingDB, dpDB, true, true);
