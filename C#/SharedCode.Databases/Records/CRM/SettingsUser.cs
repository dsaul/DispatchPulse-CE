...
{
	public record SettingsUser(Guid? Id, string? Json, string? SearchString, string? LastModifiedIso8601) : JSONTable(Id, Json, SearchString, LastModifiedIso8601)
	{
		public static Dictionary<Guid, SettingsUser> ForId(NpgsqlConnection connection, Guid id) {

			Dictionary<Guid, SettingsUser> ret = new Dictionary<Guid, SettingsUser>();

			string sql = @"SELECT * from ""settings-user"" WHERE id = @id";
			...
			if (reader.HasRows) {
				while (reader.Read()) {
					SettingsUser obj = SettingsUser.FromDataReader(reader);
					if (obj.Id == null) {
						continue;
					}
		...
		public static Dictionary<Guid, SettingsUser> All(NpgsqlConnection connection) {

			Dictionary<Guid, SettingsUser> ret = new Dictionary<Guid, SettingsUser>();

			string sql = @"SELECT * from ""settings-user""";
			using NpgsqlCommand cmd = new NpgsqlCommand(sql, connection);
		...
		public static Dictionary<Guid, SettingsUser> ForIds(NpgsqlConnection connection, IEnumerable<Guid> ids) {
			...
			string sql = $"SELECT * from \"settings-user\" WHERE id IN ({string.Join(", ", valNames)})";
			using NpgsqlCommand cmd = new NpgsqlCommand(sql, connection);
		...
		public static List<Guid> Delete(NpgsqlConnection connection, List<Guid> idsToDelete) {
			...
			string sql = $"DELETE FROM \"settings-user\" WHERE \"id\" IN ({string.Join(", ", valNames)})";
			using NpgsqlCommand cmd = new NpgsqlCommand(sql, connection);
			for (int i = 0; i < valNames.Count; i++) {
		...
		public static void Upsert(
			...
			Dictionary<Guid, SettingsUser> updateObjects,
			out List<Guid> callerResponse,
			out Dictionary<Guid, SettingsUser> toSendToOthers,
			bool printDots = false
			) {

			callerResponse = new List<Guid>();
			toSendToOthers = new Dictionary<Guid, SettingsUser>();

			foreach (KeyValuePair<Guid, SettingsUser> kvp in updateObjects) {
		...
		public static SettingsUser FromDataReader(NpgsqlDataReader reader) {
...
#warning TODO: Implement
		...
		public static void VerifyRepairTable(NpgsqlConnection dpDB, bool insertDefaultContents = false) {
			...
			if (dpDB.TableExists("settings-user")) {
				Log.Debug($"----- Table \"settings-user\" exists.");
			} else {
				Log.Debug($"----- Table \"settings-user\" doesn't exist, creating.");

				using NpgsqlCommand cmd = new NpgsqlCommand(@"
					CREATE TABLE ""public"".""settings-user"" (
						...
						CONSTRAINT ""settings_user_pk"" PRIMARY KEY(""id"")
					) WITH(oids = false);
					", dpDB);
				cmd.ExecuteNonQuery();
			}
