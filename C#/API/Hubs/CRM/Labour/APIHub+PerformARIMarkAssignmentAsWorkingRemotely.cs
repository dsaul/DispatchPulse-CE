...
namespace API.Hubs
{
	public partial class APIHub : Hub
	{
		...
		public async Task<PerformARIMarkAssignmentAsWorkingRemotelyResponse> PerformARIMarkAssignmentAsWorkingRemotely(PerformARIMarkAssignmentAsWorkingRemotelyParams p)
		{
			if (string.IsNullOrWhiteSpace(SharedCode.Hubs.Konstants.ARI_AND_API_SHARED_SECRET))
				return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: true, ErrorMessage: "ARI_AND_API_SHARED_SECRET_FILE not set!", Completed: false);
			if (p.SharedSecret != SharedCode.Hubs.Konstants.ARI_AND_API_SHARED_SECRET)
				return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: true, ErrorMessage: "Shared Secret doesn't match.", Completed: false);
			if (null == p)
				throw new ArgumentNullException(nameof(p));
			if (string.IsNullOrWhiteSpace(p.CompanyPhoneId))
				return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: true, ErrorMessage: "Didn't receive a company phone id.", Completed: false);
			if (string.IsNullOrWhiteSpace(p.AgentPhoneId))
				return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: true, ErrorMessage: "Didn't receive a agent phone id.", Completed: false);
			if (string.IsNullOrWhiteSpace(p.EnteredPasscode))
				return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: true, ErrorMessage: "Didn't receive a agent passcode.", Completed: false);
			if (null == p.AssignmentId)
				return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: true, ErrorMessage: "Didn't receive a assignment id.", Completed: false);
			...
			do
			{
				...
				Labour labour = new Labour(
					...
					Json: new JObject
					{
						...
						[Labour.kJsonKeyLocationType] = Labour.kJsonValueLocationTypeRemote,
			...
			return new PerformARIMarkAssignmentAsWorkingRemotelyResponse(IsError: isError, ErrorMessage: errorMessage, Completed: completed);
		}
	}
}
