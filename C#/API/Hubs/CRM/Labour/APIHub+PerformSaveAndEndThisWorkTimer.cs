...
namespace API.Hubs
{
	public partial class APIHub : Hub
	{
		...
		public async Task<PerformSaveAndEndThisWorkTimerResponse> PerformSaveAndEndThisWorkTimer(PerformSaveAndEndThisWorkTimerParams p)
		{
			if (string.IsNullOrWhiteSpace(SharedCode.Hubs.Konstants.ARI_AND_API_SHARED_SECRET))
				return new PerformSaveAndEndThisWorkTimerResponse(IsError: true, ErrorMessage: "ARI_AND_API_SHARED_SECRET_FILE not set!", Completed: false);
			if (p.SharedSecret != SharedCode.Hubs.Konstants.ARI_AND_API_SHARED_SECRET)
				return new PerformSaveAndEndThisWorkTimerResponse(IsError: true, ErrorMessage: "Shared Secret doesn't match.", Completed: false);
			if (null == p)
				throw new ArgumentNullException(nameof(p));
			if (string.IsNullOrWhiteSpace(p.CompanyPhoneId))
				return new PerformSaveAndEndThisWorkTimerResponse(IsError: true, ErrorMessage: "Didn't receive a company phone id.", Completed: false);
			if (string.IsNullOrWhiteSpace(p.AgentPhoneId))
				return new PerformSaveAndEndThisWorkTimerResponse(IsError: true, ErrorMessage: "Didn't receive a agent phone id.", Completed: false);
			if (string.IsNullOrWhiteSpace(p.EnteredPasscode))
				return new PerformSaveAndEndThisWorkTimerResponse(IsError: true, ErrorMessage: "Didn't receive a agent passcode.", Completed: false);
			if (null == p.LabourId)
				return new PerformSaveAndEndThisWorkTimerResponse(IsError: true, ErrorMessage: "Didn't receive a labour id.", Completed: false);
			...
			return new PerformSaveAndEndThisWorkTimerResponse(IsError: isError, ErrorMessage: errorMessage, Completed: completed);
		}
	}
}
