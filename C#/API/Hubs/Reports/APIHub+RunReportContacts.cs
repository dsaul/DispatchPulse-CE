...
namespace API.Hubs
{
	public partial class APIHub : Hub
	{
		...
		public class RunReportContactsResponse : PermissionsIdempotencyResponse
		{
			public Guid? TaskId { get; set; }

		}

		public async Task RunReportContacts(RunReportContactsParams p)
		{
			if (null == p)
				throw new ArgumentNullException(nameof(p));

			RunReportContactsResponse response = new RunReportContactsResponse();
			...
			do
			{
				...
				if (!permissions.Contains(EnvDatabases.kPermCRMReportContactsPDF)
					)
				{
					response.IsError = true;
				...
				JobRunnerJob job = new JobRunnerJob(jobId, new JObject
				{
					...
					[JobRunnerJob.kJobsJsonKeyJobType] = JobRunnerJob.kJobTypeValueRunReportContacts,
					...
					[JobRunnerJob.kJobsJsonKeyContactIds] = JArray.FromObject(p.ContactIds),
					[JobRunnerJob.kJobsJsonKeyRunOnAllContacts] = p.RunOnAllContacts,
			...
			if (null == billingContact)
			{
				await Clients.Caller.SendAsync("RunReportContactsCB", response).ConfigureAwait(false);
			}
			else
			{
				await Clients.Group(BillingContacts.UserGroupNameForBillingContact(billingContact)).SendAsync("RunReportContactsCB", response).ConfigureAwait(false);
			}
