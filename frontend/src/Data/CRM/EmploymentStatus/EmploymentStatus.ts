...
export class EmploymentStatus {
	...
	// RPC Methods
	public static RequestAgentsEmploymentStatus = 
		RPCMethod.Register<RPCRequestAgentsEmploymentStatus>(new RPCRequestAgentsEmploymentStatus());
	public static PushAgentsEmploymentStatus = 
		RPCMethod.Register<RPCPushAgentsEmploymentStatus>(new RPCPushAgentsEmploymentStatus());
	public static DeleteAgentsEmploymentStatus = 
		RPCMethod.Register<RPCDeleteAgentsEmploymentStatus>(new RPCDeleteAgentsEmploymentStatus());
		...
		if (rtrNew.completeRequestPromise) {
			...
			rtrNew.completeRequestPromise.then(() => {
				if (ret._completeRequestPromiseResolve) {
					ret._completeRequestPromiseResolve(EmploymentStatus.ForId(id));
				}
			});
	...
	public static GetMerged(mergeValues: Record<string, any>): IEmploymentStatus {
	...
	public static ForId(id: string | null): IEmploymentStatus | null {
		...
		const statuses = store.state.Database.agentsEmploymentStatus as Record<string, IEmploymentStatus>;
	...
	public static All(): IEmploymentStatus[] {
		...
		const dblist = store.state.Database.agentsEmploymentStatus as Record<string, IEmploymentStatus>;
		
		const filtered = _.filter(
			dblist,
			(o: IEmploymentStatus) => {// eslint-disable-line @typescript-eslint/no-unused-vars
				return true;
			});
	...
	public static PermCRMExportEmploymentStatusCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.export.employment-status-definitions-csv') !== -1;
		//console.log(perms, ret);
		return ret;
