...
export class Contact {
	// RPC Methods
	public static RequestContacts = RPCMethod.Register<RPCRequestContacts>(
		new RPCRequestContacts()
	);
	public static DeleteContacts = RPCMethod.Register<RPCDeleteContacts>(
		new RPCDeleteContacts()
	);
	public static PushContacts = RPCMethod.Register<RPCPushContacts>(
		new RPCPushContacts()
	);
	...
	public static ForId(id: string | null): IContact | null {
		...
		const contacts = store.state.Database.contacts as Record<
			string,
			IContact
		>;
	...
	public static ForCompanyId(companyId: string): IContact[] {
		...
		ret = _.filter(store.state.Database.contacts, (o: IContact) => {
	...
	public static PermCRMReportContactsPDF(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.report.contacts-pdf") !== -1;
		//console.log(perms, ret);
		return ret;
	}
	...
	public static PermCRMExportContactsCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.export.contacts-csv") !== -1;
		//console.log(perms, ret);
		return ret;
	}
}
