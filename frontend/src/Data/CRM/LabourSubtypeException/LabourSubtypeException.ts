...
export class LabourSubtypeException {
	// RPC Methods
	public static RequestLabourSubtypeException = RPCMethod.Register<
		RPCRequestLabourSubtypeException
	>(new RPCRequestLabourSubtypeException());
	public static DeleteLabourSubtypeException = RPCMethod.Register<
		RPCDeleteLabourSubtypeException
	>(new RPCDeleteLabourSubtypeException());
	public static PushLabourSubtypeException = RPCMethod.Register<
		RPCPushLabourSubtypeException
	>(new RPCPushLabourSubtypeException());
			...
			rtrNew.completeRequestPromise.then(() => {
				if (ret._completeRequestPromiseResolve) {
					ret._completeRequestPromiseResolve(
						LabourSubtypeException.ForId(id)
					);
				}
			});
	...
	public static ForId(id: string | null): ILabourSubtypeException | null {
		...
		const types = store.state.Database.labourSubtypeException as Record<
			string,
			ILabourSubtypeException
		>;
	...
	public static All(): ILabourSubtypeException[] {
		const dblist = store.state.Database.labourSubtypeException as Record<
			string,
			ILabourSubtypeException
		>;

		const filtered = _.filter(dblist, (o: ILabourSubtypeException) => {
	...
	public static NameForId(id: string | null): string | null {
		try {
			...
			const all = store.state.Database.labourSubtypeException as Record<
				string,
				ILabourSubtypeException
			>;
	...
	public static PermLabourSubtypeExceptionCanRequest(): boolean {
		...
		const ret =
			...
			perms.indexOf("crm.labour-subtype-exception.request-company") !==
				-1 ||
