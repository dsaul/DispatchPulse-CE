...
export interface IDeleteLabourSubtypeExceptionPayload
...
export class RPCDeleteLabourSubtypeException extends RPCMethod {
	public Send(
		payload: IDeleteLabourSubtypeExceptionPayload
	): IRoundTripRequest {
	...
	public RecieveDefaultAction(
		...
		payload: IDeleteLabourSubtypeExceptionCB
	): boolean {
		if (
			payload.labourSubtypeExceptionDelete &&
			payload.labourSubtypeExceptionDelete.length > 0
		) {
			// Default action
			store.commit(
				"DeleteLabourSubtypeHolidaysRemote",
				payload.labourSubtypeExceptionDelete
			);
		}
