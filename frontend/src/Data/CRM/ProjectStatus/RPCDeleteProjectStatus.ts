...
export class RPCDeleteProjectStatus extends RPCMethod {
	...
	public RecieveDefaultAction(rtr: IRoundTripRequest, payload: IDeleteProjectStatusCB): boolean {
		
		if (payload.projectStatusDelete && payload.projectStatusDelete.length > 0) {
			// Default action
			store.commit('DeleteProjectStatusRemote', payload.projectStatusDelete);
		}
