...
export class ProjectNote {
	// RPC Methods
	public static RequestProjectNotes = RPCMethod.Register<
		RPCRequestProjectNotes
	>(new RPCRequestProjectNotes());
	public static DeleteProjectNotes = RPCMethod.Register<
		RPCDeleteProjectNotes
	>(new RPCDeleteProjectNotes());
	public static PushProjectNotes = RPCMethod.Register<RPCPushProjectNotes>(
		new RPCPushProjectNotes()
	);
	...
	public static ValidateObject(o: IProjectNote): IProjectNote {
		if (!o.lastModifiedISO8601 || IsNullOrEmpty(o.lastModifiedISO8601)) {
			if (
				o.json.lastModifiedISO8601 &&
				!IsNullOrEmpty(o.json.lastModifiedISO8601)
			) {
				// tslint:disable-line
		...
		if (
			...
			if (
				...
				!IsNullOrEmpty(o.lastModifiedISO8601)
			) {
		...
		if (
			!o.json.originalBillingId ||
			IsNullOrEmpty(o.json.originalBillingId)
		) {
			if (
				o.json.lastModifiedBillingId &&
				!IsNullOrEmpty(o.json.lastModifiedBillingId)
			) {
	...
	public static ForId(id: string | null): IProjectNote | null {
		...
		const statuses = store.state.Database.projectNotes as Record<
			string,
			IProjectNote
		>;
	...
	public static ForProjectIds(projectIds: string[]): IProjectNote[] {
		const all: Record<string, IProjectNote> =
			store.state.Database.projectNotes;
		if (!all) {
			return [];
		}
