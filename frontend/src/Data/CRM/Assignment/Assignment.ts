...
export class Assignment {
	...
	// RPC Methods
	public static RequestAssignments = RPCMethod.Register<RPCRequestAssignments>(new RPCRequestAssignments());
	public static DeleteAssignments = RPCMethod.Register<RPCDeleteAssignments>(new RPCDeleteAssignments());
	public static PushAssignments = RPCMethod.Register<RPCPushAssignments>(new RPCPushAssignments());
	...
	public static ValidateObject(o: IAssignment): IAssignment {
		...
		if (o.json.agentId && !IsNullOrEmpty(o.json.agentId)) {// tslint:disable-line: deprecation
	...
	public static ForId(id: string | null): IAssignment | null {
		...
		const assignments = store.state.Database.assignments as Record<string, IAssignment>;
	...
	public static ForSchedulerMinimization(
		params: {
			date: string;
		...
		const filtered = _.filter(
			...
			(o: IAssignment) => {
				...
				if (!params.isSecondPM && startTimeMode === 'afternoon-second-thing') {
					return false;
				}
				...
				if (!params.isUnscheduled) {
					...
					if (startTimeMode !== 'none' && (!startISO8601 || IsNullOrEmpty(startISO8601))) {
						return false;
					}
				...
				if (!params.isUnscheduled && 
					...
					const startISO8601 = Assignment.StartISO8601ForId(o.id);
					if (startTimeMode !== 'time' || !startISO8601 || IsNullOrEmpty(startISO8601)) {
						return false;
					}
					
					const dateParsed = DateTime.fromFormat(params.date, 'yyyy-MM-dd', {
						zone: 'local',
						setZone: true,
					});
					...
					if (+assginmentStartLocal <= +startRange || +assginmentStartLocal >= +endRange) {
						//console.error('nope!');
						return false;
					}
	...
	public static ForSchedulerCell(
		...
		const filtered = _.filter(
			...
			(o: IAssignment) => {
				...
				if (agentId && !IsNullOrEmpty(agentId)) { // cell has agent
		...
		for (const assignment of filtered) {
			...
			const invalidAgentIds = _.filter(assignment.json.agentIds, (value) => {
			...
			if (!agentId || IsNullOrEmpty(agentId)) { // this cell is unassigned
				...
				if (validAgentIds.length === 0) {
					...
					for (const discard of invalidAgentIds) { // eslint-disable-line @typescript-eslint/no-unused-vars
						res.push({
							assignment,
							forAgentId: null,
						});
	...
	public static NameForId(id: string | null): string | null {
		...
		const assignment = Assignment.ForId(id);
		if (!assignment || !assignment.json || IsNullOrEmpty(assignment.json.projectId)) {
			return null;
		}
		...
		if (project.json && project.json.name && !IsNullOrEmpty(project.json.name)) {
	...
	public static WorkDescriptionForId(id: string | null): string | null {
		...
		if (null == assignment.json.workRequested || IsNullOrEmpty(assignment.json.workRequested)) {
			return null;
		}
	...
	public static LocationDescriptionForId(id: string | null): string | null {
		...
		if (!projectId) {
			return 'No Address. Work: ' + (Assignment.WorkDescriptionForId(id) || 'No Work Specified');
		}
	...
	public static StartOnSiteForId(id: string | null): void {
		...
		const assignment = Assignment.ForId(id);
		if (!assignment) {
			console.error('Assignment.StartOnSiteForId !assignment', assignment);
			return;
		}
	...
	public static StartRemoteForId(id: string | null): void {
		...
		const assignment = Assignment.ForId(id);
		if (!assignment) {
			console.error('Assignment.StartRemoteForId !assignment', assignment);
			return;
		}
	...
	public static StartTimeModeForId(id: string | null): 
		'none' | 'morning-first-thing' | 
		'morning-second-thing' | 'afternoon-first-thing' | 
		'afternoon-second-thing' | 'time' {
		...
		const assignment = Assignment.ForId(id);
		if (!assignment) {
			console.error('Assignment.StartTimeModeForId !assignment', assignment);
			return 'none';
		}
	...
	public static StartISO8601ForId(id: string | null): string | null {
		...
		const assignment = Assignment.ForId(id);
		if (!assignment) {
			console.error('Assignment.StartISO8601ForId !assignment', assignment);
			return 'none';
		}
	...
	public static EndTimeModeForId(id: string | null): 
		'none' | 'time' {
		...
		const assignment = Assignment.ForId(id);
		if (!assignment) {
			console.error('Assignment.EndTimeModeForId !assignment', assignment);
			return 'none';
		}
	...
	public static RemoveAgentIdFromId(id: string | null, agentId: string | null): void {
	...
	public static AddAgentIdsToId(id: string | null, agentId: string | null): void {
		...
		if (agentId != null) {
			...
			if (existingElement) {
				console.debug('Agent already exists in list, not adding a second time.');
				return;
			}
		}
	...
	public static PermCRMReportAssignmentPDF(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.report.assignments-pdf') !== -1;
		//console.log(perms, ret);
		return ret;
	...
	public static PermCRMExportAssignmentCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.export.assignments-csv') !== -1;
		//console.log(perms, ret);
		return ret;
