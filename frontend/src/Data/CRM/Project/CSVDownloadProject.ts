...
export default (project: IProject): void => {
	...
	const array = [
		["Export Type:", "Projects", "Export Version:", "CSV1"],
		[
			"id",
			"parentId",
			"lastModifiedISO8601",
		...
		["Export Type:", "ProjectNotes", "Export Version:", "CSV1"],
		[
			"id",
			"lastModifiedISO8601",
			"lastModifiedBillingId",
	...
	const allNotes = store.state.Database.projectNotes as Record<
		string,
		IProjectNote
	>;
	for (const note of Object.values(allNotes)) {
		...
		if (note.json.contentType === "checkbox") {
			...
			contentCheckboxState = (note.json.content as IProjectNoteCheckbox)
				.checkboxState
				? "YES"
				: "NO";
		}
		...
		let contentURI = "";
		if (
			note.json.contentType === "image" ||
			note.json.contentType === "video"
		) {
			contentURI = (note.json.content as
				| IProjectNoteImage
				| IProjectNoteVideo).uri;
		}
	...
	const allAssignments = store.state.Database.assignments as Record<
		string,
		IAssignment
	>;
	...
	array.push(["Export Type:", "Materials", "Export Version:", "CSV1"]);
	...
	const allMaterials = store.state.Database.materials as Record<
		string,
		IMaterial
	>;
	...
	array.push(["Export Type:", "Labour", "Export Version:", "CSV1"]);
