...
export class Project {
	// RPC Methods
	public static RequestProjects = RPCMethod.Register<RPCRequestProjects>(
		new RPCRequestProjects()
	);
	public static DeleteProjects = RPCMethod.Register<RPCDeleteProjects>(
		new RPCDeleteProjects()
	);
	public static PushProjects = RPCMethod.Register<RPCPushProjects>(
		new RPCPushProjects()
	);
	...
	public static ForId(id: string | null): IProject | null {
		...
		const projects = store.state.Database.projects as Record<
			string,
			IProject
		>;
	...
	public static EndScheduleDescriptionForId(projectId: string): string {
		...
		do {
			...
			if (["none"].indexOf(project.json.startTimeMode) !== -1) {
	...
	public static StartScheduleDescriptionForId(projectId: string): string {
		...
		do {
			...
			if (project.json.startTimeMode === "morning-first-thing") {
				...
				project.json.startTimeMode === "afternoon-second-thing"
			) {
	...
	public static UpdateForceAssignmentsToUseProjectScheduleForId(
		id: string | null,
		flag: boolean
	): void {
		if (!id) {
	...
	public static PermCRMReportProjectsPDF(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.report.projects-pdf") !== -1;
		//console.log(perms, ret);
		return ret;
	}
	...
	public static PermCRMExportProjectsCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.export.projects-csv") !== -1;
		//console.log(perms, ret);
		return ret;
	}
}
