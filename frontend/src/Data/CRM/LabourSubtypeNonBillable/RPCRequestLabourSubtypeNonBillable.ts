...
export interface IRequestLabourSubtypeNonBillablePayload
	extends IIdempotencyRequest {
	limitToIds?: string[] | null;
}

export interface IRequestLabourSubtypeNonBillableCB
...
export class RPCRequestLabourSubtypeNonBillable extends RPCMethod {
	public Send(
		payload: IRequestLabourSubtypeNonBillablePayload
	): IRoundTripRequest {
	...
	public RecieveDefaultAction(
		...
		payload: IRequestLabourSubtypeNonBillableCB
	): boolean {
		if (!payload.labourSubtypeNonBillable) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting labour subtype non-billable #2.`
					)
				);
			}
			return false;
