...
export class Agent {
	// RPC Methods
	public static RequestAgents = RPCMethod.Register<RPCRequestAgents>(
		new RPCRequestAgents()
	);
	public static DeleteAgents = RPCMethod.Register<RPCDeleteAgents>(
		new RPCDeleteAgents()
	);
	public static PushAgents = RPCMethod.Register<RPCPushAgents>(
		new RPCPushAgents()
	);
	...
	public static LoggedInAgentId(): string | null {
		...
		const billingContacts: Record<string, IBillingContacts> =
			store.state.Database.billingContacts;
		if (!billingContacts) {
			return null;
		}
	...
	public static DescriptionForAgentList(
		agents: IAgent[],
		addPeriod = true
	): string {
		let ret = "";
		...
		for (let i = 0; i < agents.length; i++) {
			...
			ret += IsNullOrEmpty(agents[i].json.name)
				? "No Name"
				: agents[i].json.name;
		}
	...
	public static ForAssignmentId(assignmentId: string | null): IAgent[] {
		...
		if (!assignment || !assignment.json || !assignment.json.agentIds) {
			return [];
		}
	...
	public static PermCRMExportAgentsCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.export.agents-csv") !== -1;
		//console.log(perms, ret);
		return ret;
	}
}
