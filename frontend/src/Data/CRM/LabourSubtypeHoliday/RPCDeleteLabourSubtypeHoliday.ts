...
export interface IDeleteLabourSubtypeHolidaysPayload
...
export class RPCDeleteLabourSubtypeHolidays extends RPCMethod {
	public Send(
		payload: IDeleteLabourSubtypeHolidaysPayload
	): IRoundTripRequest {
	...
	public RecieveDefaultAction(
		...
		payload: IDeleteLabourSubtypeHolidaysCB
	): boolean {
		if (
			payload.labourSubtypeHolidaysDelete &&
			payload.labourSubtypeHolidaysDelete.length > 0
		) {
			// Default action
			store.commit(
