...
export class LabourType {
	// RPC Methods
	public static RequestLabourTypes = RPCMethod.Register<
		RPCRequestLabourTypes
	>(new RPCRequestLabourTypes());
	public static DeleteLabourTypes = RPCMethod.Register<RPCDeleteLabourTypes>(
		new RPCDeleteLabourTypes()
	);
	public static PushLabourTypes = RPCMethod.Register<RPCPushLabourTypes>(
		new RPCPushLabourTypes()
	);
	...
	public static DefaultBillableTypeId(): string | null {
		const types = store.state.Database.labourTypes as Record<
			string,
			ILabourType
		>;
		...
		const fBillable = _.filter(types, (o: ILabourType) => {
		...
		if (fBillable.length === 0) {
			...
			const fDefault = _.filter(fBillable, (o: ILabourType) => {
	...
	public static All(): ILabourType[] {
		const dblist = store.state.Database.labourTypes as Record<
			string,
			ILabourType
		>;

		const filtered = _.filter(dblist, (o: ILabourType) => {
	...
	public static NameForId(id: string | null): string | null {
		try {
			...
			const all = store.state.Database.labourTypes as Record<
				string,
				ILabourType
			>;
