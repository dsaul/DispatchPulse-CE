...
export class Company {
	// RPC Methods
	public static RequestCompanies = RPCMethod.Register<RPCRequestCompanies>(
		new RPCRequestCompanies()
	);
	public static DeleteCompanies = RPCMethod.Register<RPCDeleteCompanies>(
		new RPCDeleteCompanies()
	);
	public static PushCompanies = RPCMethod.Register<RPCPushCompanies>(
		new RPCPushCompanies()
	);
	...
	public static ForId(id: string | null): ICompany | null {
		...
		const companies = store.state.Database.companies as Record<
			string,
			ICompany
		>;
	...
	public static CompanyListDescriptionForIds(
	...
	public static PermCRMReportCompaniesPDF(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.report.companies-pdf") !== -1;
		//console.log(perms, ret);
		return ret;
	}
	...
	public static PermCRMExportCompaniesCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = perms.indexOf("crm.export.companies-csv") !== -1;
		//console.log(perms, ret);
		return ret;
	}
}
