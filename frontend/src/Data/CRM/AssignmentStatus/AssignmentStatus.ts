...
export class AssignmentStatus {
	// RPC Methods
	public static RequestAssignmentStatus = RPCMethod.Register<
		RPCRequestAssignmentStatus
	>(new RPCRequestAssignmentStatus());
	public static DeleteAssignmentStatus = RPCMethod.Register<
		RPCDeleteAssignmentStatus
	>(new RPCDeleteAssignmentStatus());
	public static PushAssignmentStatus = RPCMethod.Register<
		RPCPushAssignmentStatus
	>(new RPCPushAssignmentStatus());
	...
	public static ForId(id: string | null): IAssignmentStatus | null {
		...
		const statuses = store.state.Database.assignmentStatus as Record<
			string,
			IAssignmentStatus
		>;
	...
	public static PermCRMExportAssignmentStatusCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret =
			perms.indexOf("crm.export.assignment-status-definitions-csv") !==
			-1;
		//console.log(perms, ret);
		return ret;
	}
