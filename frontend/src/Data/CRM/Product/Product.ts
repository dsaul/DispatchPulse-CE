...
export class Product {
	// RPC Methods
	public static RequestProducts = RPCMethod.Register<RPCRequestProducts>(new RPCRequestProducts());
	public static DeleteProducts = RPCMethod.Register<RPCDeleteProducts>(new RPCDeleteProducts());
	public static PushProducts = RPCMethod.Register<RPCPushProducts>(new RPCPushProducts());
	...
	public static ForId(id: string | null): IProduct | null {
		...
		const products = store.state.Database.products as Record<string, IProduct>;
	...
	public static PermCRMExportProductsCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.export.product-definitions-csv') !== -1;
		//console.log(perms, ret);
		return ret;
