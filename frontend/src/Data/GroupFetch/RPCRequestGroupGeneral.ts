...
export class RPCRequestGroupGeneral extends RPCMethod {
	...
	public RecieveDefaultAction(
		...
		payload: IRequestGroupGeneralCB
	): boolean {
		if (!payload.assignmentStatus) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve assignment status.`
					)
				);
			}
			return false;
		...
		if (!payload.agentsEmploymentStatus) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve employment status.`
					)
				);
			}
			return false;
		...
		if (!payload.estimatingManHours) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve man hours.`
					)
				);
			}
			return false;
		...
		if (!payload.labourSubtypeHolidays) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve labour subtype holidays.`
					)
				);
			}
			return false;
		...
		if (!payload.labourSubtypeException) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve labour subtype exception.`
					)
				);
			}
			return false;
		...
		if (!payload.labourSubtypeNonBillable) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve labour subtype non-billable.`
					)
				);
			}
			return false;
		...
		if (!payload.labourTypes) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve labour types.`
					)
				);
			}
			return false;
		...
		if (!payload.projectStatus) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve project status.`
					)
				);
			}
			return false;
		...
		if (!payload.settingsDefault) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve settings default.`
					)
				);
			}
			return false;
		...
		if (!payload.settingsProvisioning) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve settings provisioning.`
					)
				);
			}
			return false;
		...
		if (!payload.settingsUser) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "general", didn't recieve settings user.`
					)
				);
			}
			return false;
		...
		store.commit(
			...
			payload.agentsEmploymentStatus
		);
		store.commit(
		...
		store.commit(
			...
			payload.labourSubtypeNonBillable
		);
