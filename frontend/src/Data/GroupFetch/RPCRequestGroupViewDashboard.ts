...
export class RPCRequestGroupViewDashboard extends RPCMethod {
	...
	public RecieveDefaultAction(
		...
		payload: IRequestGroupViewDashboardCB
	): boolean {
		if (!payload.agents) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "dashboard", didn't recieve agents.`
					)
				);
			}
			return false;
		...
		if (!payload.assignments) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "dashboard", didn't recieve assignments.`
					)
				);
			}
			return false;
		...
		if (!payload.labour) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "dashboard", didn't recieve labour.`
					)
				);
			}
			return false;
		...
		if (!payload.projects) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting group "dashboard", didn't recieve projects.`
					)
				);
			}
			return false;
