...
export class BillingPackages {
	// RPC Methods
	public static RequestBillingPackagesForCurrentSession = RPCMethod.Register<
		RPCRequestBillingPackagesForCurrentSession
	>(new RPCRequestBillingPackagesForCurrentSession());
	...
	public static ForId(packageId: string): IBillingPackages | null {
		...
		const billingPackages: Record<string, IBillingPackages> =
			store.state.Database.billingPackages;
		if (!billingPackages) {
			return null;
		}
