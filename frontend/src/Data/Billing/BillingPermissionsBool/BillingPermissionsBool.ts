...
export class BillingPermissionsBool {
	// RPC Methods
	public static RequestBillingPermissionsBoolForCurrentSession = RPCMethod.Register<
		RPCRequestBillingPermissionsBoolForCurrentSession
	>(new RPCRequestBillingPermissionsBoolForCurrentSession());
	public static PerformBillingPermissionsBoolAdd = RPCMethod.Register<
		RPCPerformBillingPermissionsBoolAdd
	>(new RPCPerformBillingPermissionsBoolAdd());
	public static PerformBillingPermissionsBoolRemove = RPCMethod.Register<
		RPCPerformBillingPermissionsBoolRemove
	...
	public static AllForBillingContactId(): string[] {
		...
		const ret = BillingPermissionsBool.KeysForBillingContactIdAmalgamated(
			contactId
		);
	...
	public static KeysForBillingContactIdAmalgamated(
		contactId: string
	): string[] {
		const ret: string[] = [];
		...
		const userPermissions = BillingPermissionsBool.ForBillingContactId(
			contactId
		);
		//console.debug('userPermissions', userPermissions);

		for (const perm of userPermissions) {
			if (perm.value === true && ret.indexOf(perm.key) === -1) {
				ret.push(perm.key);
			}
		}

		// Next we find if they are in any groups.
		const groups = BillingPermissionsGroupsMemberships.ForBillingContactId(
			contactId
		);
		const groupIds = [];
