...
export class BillingPermissionsGroups {
	// RPC Methods
	public static RequestBillingPermissionsGroupsForCurrentSession = RPCMethod.Register<
		RPCRequestBillingPermissionsGroupsForCurrentSession
	...
	public static All(): Record<string, IBillingPermissionsGroups> | null {
		const groups = store.state.Database.billingPermissionsGroups as Record<
			string,
			IBillingPermissionsGroups
		>;
	...
	public static GroupIdsForBillingId(id: string | null): string[] {
		...
		const ret = [];
		const memberships = BillingPermissionsGroupsMemberships.ForBillingContactId(
			id
		);
