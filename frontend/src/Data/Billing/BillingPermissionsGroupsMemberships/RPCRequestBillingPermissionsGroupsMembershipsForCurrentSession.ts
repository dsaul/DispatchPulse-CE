...
export interface IRequestBillingPermissionsGroupsMembershipsForCurrentSessionCB
...
export class RPCRequestBillingPermissionsGroupsMembershipsForCurrentSession extends RPCMethod {
	public Send(
		payload: IRequestBillingPermissionsGroupsMembershipsForCurrentSessionPayload
	): IRoundTripRequest {
		...
		if (!payload.billingPermissionsGroupsMemberships) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(
						`Error requesting permissions groups memberships #2.`
					)
				);
			}
			return false;
