<template>
	<v-card flat>
		...
		<v-text-field autocomplete="newpassword" class="mx-4 e2e-products-data-table-filter" v-model="searchString"
			hide-details label="Filter" prepend-inner-icon="search" solo style="margin-bottom: 10px;"></v-text-field>


		<v-data-table v-if="PermProductsCanRequest()" :headers="headers" :items="AllProducts" :search="searchString"
			...
			<template v-slot:[`item.action`]="{ item }">
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item @click="EditEntry(item)" :disabled="disabled || !PermProductsCanPush()">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
						...
						<v-list-item @click="DeleteEntry(item)" :disabled="disabled || !PermProductsCanDelete()">
							<v-list-item-icon>
