<template>
	<v-card flat>
		...
		<v-text-field autocomplete="newpassword" class="mx-4 e2e-labour-non-billable-data-table-filter"
			v-model="searchString" hide-details label="Filter" prepend-inner-icon="search" solo
			style="margin-bottom: 10px;"></v-text-field>


		<v-data-table v-if="PermLabourSubtypeNonBillableCanRequest()" :headers="headers" :items="AllLabourNonBillable"
