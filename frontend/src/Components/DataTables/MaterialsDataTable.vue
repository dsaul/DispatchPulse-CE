<template>
	<v-card flat>
		...
		<v-text-field autocomplete="newpassword" class="mx-4 e2e-materials-data-table-filter" v-model="searchString"
			hide-details label="Filter" prepend-inner-icon="search" solo style="margin-bottom: 10px;"></v-text-field>


		<v-data-table v-if="PermMaterialsCanRequest()" :headers="headers" :items="AllMaterials" :search="searchString"
			item-key="name" :custom-filter="CustomDataTableFilter" ref="dataTable" :footer-props="{
			...
			<template v-slot:[`item.json.projectId`]="{ item }">
				<DataLoaderProject :projectId="item.json.projectId">
					<router-link v-if="item && item.json && item.json.projectId"
			...
			<template v-slot:[`item.action`]="{ item }">
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item @click="EditEntry(item)" :disabled="disabled || !PermMaterialsCanPush()">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
						...
						<v-list-item @click="DeleteEntry(item)" :disabled="disabled || !PermMaterialsCanDelete()">
							<v-list-item-icon>
