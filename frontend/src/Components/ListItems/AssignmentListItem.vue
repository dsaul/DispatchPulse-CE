				...
				<v-list-item-title
					...
					<span v-if="Project && Project.json && Project.json.addresses">
						<span v-for="(addr, index) of Project.json.addresses" :key="addr.id">
							<span v-if="(index == Project.json.addresses.length - 1) && Project.json.addresses.length > 1">and </span>
							{{addr.value}}<!--
							--><span v-if="!addr.value.endsWith('.')">.</span>
							<span v-if="(index != Project.json.addresses.length - 1) && Project.json.addresses.length > 1">, </span>
			...
			<v-list-item-action v-if="showMenuButton">
				<v-menu bottom left>
					...
					<v-list dense>
						...
						<v-list-item
							@click="StartTravel()"
							:disabled="disabled || !PermLabourCanPushSelf()"
							>
							<v-list-item-icon>
						...
						<v-list-item
							:disabled="isDialogue || disabled || !PermAssignmentCanRequest()"
