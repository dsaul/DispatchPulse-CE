		...
		<v-list-item :class="{ paddingLeftRight5px: reducePadding, }">
			...
			<v-list-item-content @click="ClickEntry">
				...
				<v-list-item-subtitle style="width: 1px; /*to force flex to allow this to get smaller*/">
					<v-chip v-if="showEmploymentStatus && EmploymentStatusName" label outlined small
					...
					<v-chip v-if="showTitle && value && value.json && value.json.title" label outlined small
						style="margin-right: 5px;">
						{{ value.json.title }}
					</v-chip>
			...
			<v-list-item-action v-if="showMenuButton">
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item :disabled="isDialogue || disabled || !PermAgentsCanRequest()"
