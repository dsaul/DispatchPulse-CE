		...
		<v-list-item>
			...
			<v-list-item-content @click="ClickEntry">
				...
				<v-list-item-subtitle style="width: 1px; /*to force flex to allow this to get smaller*/">
					<v-tooltip v-if="VoicemailDisplayTypeForId(value.id)" top>
					...
					<v-tooltip v-if="VoicemailDisplayCallerIdNameForId(value.id)" top>
						<template v-slot:activator="{ on }" v-on="on">
							<v-chip label outlined small style="margin-right: 5px;" v-on="on">
					...
					<v-tooltip v-if="VoicemailDisplayCallerIdNumberForId(value.id)" top>
						<template v-slot:activator="{ on }" v-on="on">
							<v-chip label outlined small style="margin-right: 5px;" v-on="on">
					...
					<v-tooltip v-if="VoicemailDisplayCallbackNumberForId(value.id)" top>
						<template v-slot:activator="{ on }" v-on="on">
							<v-chip label outlined small style="margin-right: 5px;" v-on="on">
			...
			<v-list-item-action v-if="showMenuButton">
				<v-menu bottom left>
					...
					<v-list dense>
						...
						<v-list-item @click="$emit('delete-entry', value)"
