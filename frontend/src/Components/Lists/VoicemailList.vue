<template>
	<div>
		<v-list v-if="PermVoicemailsCanRequest()">
			<v-text-field v-if="showFilter" autocomplete="newpassword" class="mx-4 e2e-voicemails-list-filter"
			...
			<div v-if="PageRows.length != 0">
				...
				<v-list-item-group color="primary">
					<VoicemailListItem v-for="(row, index) in PageRows" :key="row.id" v-model="PageRows[index]"
						:showMenuButton="showMenuButton" :isDialogue="isDialogue" @ClickEntry="ClickEntry"
						@OpenEntry="OpenEntry" @delete-entry="OpenDeleteVoicemail" :disabled="disabled" />
				</v-list-item-group>
