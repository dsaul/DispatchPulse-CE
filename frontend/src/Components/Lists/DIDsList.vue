<template>
	<div>
		<v-list v-if="PermDIDsCanRequest()">
			<v-text-field v-if="showFilter" autocomplete="newpassword" class="mx-4 e2e-dids-list-filter"
				v-model="filter" hide-details label="Filter" prepend-inner-icon="search" solo
				style="margin-bottom: 10px;" ref="filterField">
			</v-text-field>
			...
			<div v-if="PageRows.length != 0">
				...
				<v-list-item-group color="primary">
					<DIDsListItem v-for="(row, index) in PageRows" :key="row.id" v-model="PageRows[index]"
						:showMenuButton="showMenuButton" :isDialogue="isDialogue" @ClickEntry="ClickEntry"
						@OpenEntry="OpenEntry" @delete-entry="OpenDeleteDID" :disabled="disabled" />
				</v-list-item-group>
		...
		<DeleteDIDDialogue v-model="deleteDIDModel" :isOpen="deleteDIDOpen" @Delete="SaveDeleteDID"
			@Cancel="CancelDeleteDID" ref="deleteDIDDialogue" />
	</div>
</template>
