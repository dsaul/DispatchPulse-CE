<template>
	<div>
		<v-list v-if="PermAgentsCanRequest()">
			<v-text-field v-if="showFilter" autocomplete="newpassword" class="mx-4 e2e-agent-list-filter"
			...
			<div v-if="PageRows.length != 0">
				...
				<v-list-item-group color="primary">
					<div v-for="(row, index) in PageRows" :key="row.id">
						...
						<AgentsListItem v-else v-model="PageRows[index]" :showMenuButton="showMenuButton"
							:isDialogue="isDialogue" @ClickEntry="ClickEntry" @OpenEntry="OpenEntry"
							@DeleteEntry="DeleteEntry" :disabled="disabled" />
					</div>
				</v-list-item-group>
