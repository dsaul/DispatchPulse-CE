		...
		<v-dialog v-model="selectDialogOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
			...
			<v-card>
				...
				<v-card-text>
					...
					<AgentList :openEntryOnClick="false" :showMenuButton="false" @ClickEntry="OnClickEntry"
						class="e2e-agent-select-list" ref="list" />
					...
					<v-dialog v-model="newDialogueVisible" persistent scrollable :fullscreen="MobileDeviceWidth()">
						<v-card>
							...
							<v-divider></v-divider>
							<v-card-text>
								<AgentEditor ref="editor" v-model="newDialogueObject" :showAppBar="false"
									:showFooter="false" preselectTabName="Admin" :isMakingNew="true" />
							</v-card-text>
							<v-divider></v-divider>
							<v-card-actions>
								<v-spacer />
		...
		<v-input :messages="[]" ref="input" persistent-hint :hint="hint" :required="required" v-model="value"
			...
			<template v-slot:append>
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item @click="OpenContact()" :disabled="isDialogue || IsValueEmpty || disabled">
							<v-list-item-icon>
						...
						<v-divider />
						<v-list-item @click="SelectNewAgent()" :disabled="disabled || readonly">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
						...
						<!-- <div v-if="index"> -->
						<v-divider />
						<v-list-item @click="$emit('MoveUp', index)" :disabled="isFirstIndex || disabled || readonly">
							<v-list-item-icon>
						...
						<v-list-item @click="$emit('MoveDown', index)" :disabled="isLastIndex || disabled || readonly">
							<v-list-item-icon>
						...
						<v-list-item @click="$emit('InsertNewRowAtIndex', index + 1)" :disabled="disabled || readonly">
							<v-list-item-icon>
								<v-icon>add</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
			...
			<div
				...
				<div v-if="!Agent && value">
					{{ value }} (can't find)
				</div>
				<div v-else-if="Agent" @click="SelectNewAgent()" :style="{
			flex: '1',
			display: 'flex',
			...
			color: disabled ? 'rgba(0, 0, 0, 0.38)' : 'inherit',
				...
				<div v-else @click="SelectNewAgent()" style="flex: 1; text-align: center;"
					class="e2e-agent-select-field-select-or-add-agent">
					<v-btn color="primary" text :disabled="disabled || readonly">
						Select or Add Agent
					</v-btn>
				</div>
				<div v-if="Agent && !isDialogue" class="d-none d-sm-flex">
					<v-tooltip top>
						<template v-slot:activator="{ on }">
							<v-chip :to="`/section/agents/${value}?tab=Agenda`" color="primary" label outlined
