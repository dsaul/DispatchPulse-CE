<template>
	<div :class="{
		...
		<v-dialog v-model="selectDialogOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
			...
			<v-card>
				...
				<v-card-text>
					...
					<CompanyList :showMenuButton="false" :openEntryOnClick="false" @ClickEntry="OnClickEntry"
						class="e2e-company-select-list" ref="list" />
					...
					<v-dialog v-model="newDialogueVisible" persistent scrollable :fullscreen="MobileDeviceWidth()">
						<v-card>
							...
							<v-divider></v-divider>
							<v-card-text>
								<CompanyEditor ref="editor" v-model="newDialogueObject" :showAppBar="false"
		...
		<v-input :messages="[]" ref="input" persistent-hint :hint="hint" :required="required" v-model="value"
			...
			<template v-slot:append>
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item @click="OpenCompany()" :disabled="isDialogue || IsValueEmpty || disabled">
							<v-list-item-icon>
						...
						<v-divider />
						<v-list-item @click="SelectNewCompany()" :disabled="disabled || readonly">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
			...
			<div
				...
				<div v-if="!Company && value">
					{{ value }} (can't find)
				</div>
				<div v-else-if="CompanyForId(value)" @click="SelectNewCompany()" :style="{
		flex: '1',
		display: 'flex',
		...
		color: disabled ? 'rgba(0, 0, 0, 0.38)' : 'inherit',
				...
				<div v-else @click="SelectNewCompany()" style="flex: 1; text-align: center;"
				...
				<div v-if="CompanyForId(value) && !isDialogue" class="d-none d-sm-flex">
					<v-tooltip top>
						<template v-slot:activator="{ on }">
							<v-chip :to="`/section/companies/${value}?tab=General`" color="primary" label outlined
		...
		<div v-if="showCompanyContactsBelow == true && value != null">
			<div v-for="contact of ContactsForCompanyId(value)" :key="contact.id" style="font-size:14px;">
				<router-link :to="`/section/contacts/${contact.id}`">{{ contact.json.name }} <span
						v-if="contact.json.title">
						&mdash; {{ contact.json.title }}</span></router-link>
			</div>
