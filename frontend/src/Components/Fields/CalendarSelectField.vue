<template>
	<div class="outerdiv" style="border-bottom: ">
		<v-dialog v-model="selectDialogOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
			...
			<v-card>
				<v-card-title>Select a Calendar</v-card-title>
				<v-divider></v-divider>
				<v-card-text>

					<CalendarList :openEntryOnClick="false" :showMenuButton="false" @ClickEntry="OnClickEntry"
						ref="list" />

					<v-dialog v-model="newDialogueVisible" persistent scrollable :fullscreen="MobileDeviceWidth()">
						<v-card>
							<v-card-title>Create and Select New Calendar</v-card-title>
							<v-divider></v-divider>
							<v-card-text>
								<CalendarEditor ref="editor" v-model="newDialogueObject" :showAppBar="false"
		...
		<v-input :messages="[]" ref="input" persistent-hint :hint="hint" :required="required" v-model="value"
			...
			<template v-slot:append>
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item @click="OpenCalendar()" :disabled="isDialogue || IsValueEmpty || disabled">
							<v-list-item-icon>
						...
						<v-divider />
						<v-list-item @click="SelectNewCalendar()" :disabled="disabled || readonly">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
			...
			<div
				...
				<div v-if="!Calendar && value">
					{{ value }} (can't find)
				</div>
				<div v-else-if="CalendarForId(value)" @click="SelectNewCalendar()" :style="{
			flex: '1',
			display: 'flex',
			'align-items': 'flex-end',
			color: disabled ? 'rgba(0, 0, 0, 0.38)' : 'inherit',
		}">
					{{ CalendarNameForId(value) || 'Empty calendar name.' }}
				</div>
				<div v-else @click="SelectNewCalendar()" :disabled="disabled || readonly"
					style="flex: 1; text-align: center;">
					<v-btn color="primary" text>Select or Add Calendar</v-btn>
				</div>
				<div v-if="CalendarForId(value) && !isDialogue" class="d-none d-sm-flex">
					<v-tooltip top>
						<template v-slot:activator="{ on }">
							<v-chip :to="`/section/calendars/${value}?tab=General`" color="primary" label outlined
...
export default class CalendarSelectField extends FieldBase {

	@Prop({ default: 'Calendar' }) declare public readonly label: string | null;
	...
	protected CalendarForId = Calendar.ForId;
	protected CalendarNameForId = Calendar.NameForId;
	protected MobileDeviceWidth = MobileDeviceWidth;

	protected newDialogueVisible = false;
	protected newDialogueObject: ICalendar | null = null;
	...
	public mounted(): void {
		...
		if (null != this.value && !IsNullOrEmpty(this.value)) {
			...
			const contact = Calendar.ForId(this.value);
			if (null == contact) {
				const id = this.value;
				SignalRConnection.Ready(() => {
					BillingPermissionsBool.Ready(() => {
						Calendar.FetchForId(id);
					});
				});
	...
	protected get Calendar(): ICalendar | null {
		const contact = Calendar.ForId(this.value);
		if (!contact) {
			return null;
		}
	...
	protected SelectNewCalendar(): void {
	...
	protected OpenCalendar(): void {
		...
		if (!IsNullOrEmpty(this.value)) {
			this.$router.push(`/section/calendars/${this.value}?tab=General`).catch(((e: Error) => { }));// eslint-disable-line
		} else {
			console.error('can\'t open calendars auto attendant as IsNullOrEmpty(this.value)');
		}
	...
	protected OpenNewDialogue(): void {
		...
		this.newDialogueObject = Calendar.GetEmpty();
	...
	protected NewDialogueSaveAndAdd(): void {
		...
		if (this.newDialogueObject && this.$refs.editor.IsValidated()) {
			...
			// First add the agent.
			const state = this.newDialogueObject as ICalendar;
			if (state.id) {
...
Vue.component('CalendarSelectField', CalendarSelectField);
