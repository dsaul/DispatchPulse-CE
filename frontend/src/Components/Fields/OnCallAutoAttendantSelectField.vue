<template>
	<div class="outerdiv" style="border-bottom: ">
		<v-dialog v-model="selectDialogOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
			...
			<v-card>
				<v-card-title>Select a On-Call Responder</v-card-title>
				<v-divider></v-divider>
				<v-card-text>

					<OnCallAutoAttendantsList :openEntryOnClick="false" :showMenuButton="false"
						@ClickEntry="OnClickEntry" ref="list" />

					<v-dialog v-model="newDialogueVisible" persistent scrollable :fullscreen="MobileDeviceWidth()">
						<v-card>
							<v-card-title>Create and Select New On-Call Responder</v-card-title>
							<v-divider></v-divider>
							<v-card-text>
								<OnCallAutoAttendantEditor ref="editor" v-model="newDialogueObject" :showAppBar="false"
		...
		<v-input :messages="[]" ref="input" persistent-hint :hint="hint" :required="required" v-model="value"
			...
			<template v-slot:append>
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item @click="OpenOnCallAutoAttendant()"
						...
						<v-divider />
						<v-list-item @click="SelectNewOnCallAutoAttendant()" :disabled="disabled || readonly">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
			...
			<div
				...
				<div v-if="!OnCallAutoAttendant && value">
					{{ value }} (can't find)
				</div>
				<div v-else-if="OnCallAutoAttendantForId(value)" @click="SelectNewOnCallAutoAttendant()" :style="{
			flex: '1',
			display: 'flex',
			'align-items': 'flex-end',
			color: disabled ? 'rgba(0, 0, 0, 0.38)' : 'inherit',
		}">
					{{ OnCallAutoAttendantNameForId(value) || 'Empty on-call auto attendant name.' }}
				</div>
				<div v-else @click="SelectNewOnCallAutoAttendant()" :disabled="disabled || readonly"
					style="flex: 1; text-align: center;">
					<v-btn color="primary" text>Select or Add On-Call Responder</v-btn>
				</div>
				<div v-if="OnCallAutoAttendantForId(value) && !isDialogue" class="d-none d-sm-flex">
					<v-tooltip top>
						<template v-slot:activator="{ on }">
							<v-chip :to="`/section/on-call/${value}?tab=General`" color="primary" label outlined
...
export default class OnCallAutoAttendantSelectField extends FieldBase {

	@Prop({ default: 'On-Call Responder' }) declare public readonly label: string | null;
	@Prop({ default: false }) public readonly showCompanyContactsBelow!: boolean | null;

	public $refs!: {
		input: Vue,
		editor: OnCallAutoAttendantEditor,
		list: OnCallAutoAttendantsList,
	};

	protected OnCallAutoAttendantForId = OnCallAutoAttendant.ForId;
	protected OnCallAutoAttendantNameForId = OnCallAutoAttendant.NameForId;
	protected MobileDeviceWidth = MobileDeviceWidth;

	protected newDialogueVisible = false;
	protected newDialogueObject: IOnCallAutoAttendant | null = null;
	...
	public mounted(): void {
		...
		if (null != this.value && !IsNullOrEmpty(this.value)) {
			...
			const contact = OnCallAutoAttendant.ForId(this.value);
			if (null == contact) {
				const id = this.value;
				SignalRConnection.Ready(() => {
					BillingPermissionsBool.Ready(() => {
						OnCallAutoAttendant.FetchForId(id);
					});
				});
	...
	protected get OnCallAutoAttendant(): IOnCallAutoAttendant | null {
		const contact = OnCallAutoAttendant.ForId(this.value);
		if (!contact) {
			return null;
		}
	...
	protected SelectNewOnCallAutoAttendant(): void {
		...
		console.log('SelectNewOnCallAutoAttendant()');
	...
	protected OpenOnCallAutoAttendant(): void {
		console.log('OpenOnCallAutoAttendant()');

		if (!IsNullOrEmpty(this.value)) {
			this.$router.push(`/section/on-call/${this.value}?tab=General`).catch(((e: Error) => { }));// eslint-disable-line
		} else {
			console.error('can\'t open on-call auto attendant as IsNullOrEmpty(this.value)');
		}
	...
	protected OpenNewDialogue(): void {
		...
		this.newDialogueObject = OnCallAutoAttendant.GetEmpty();
	...
	protected NewDialogueSaveAndAdd(): void {
		...
		if (this.newDialogueObject && this.$refs.editor.IsValidated()) {
			...
			// First add the agent.
			const state = this.newDialogueObject as IOnCallAutoAttendant;
			if (state.id) {
...
Vue.component('OnCallAutoAttendantSelectField', OnCallAutoAttendantSelectField);
