	...
	<div :class="{
		...
		<v-dialog v-model="selectDialogOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
			...
			<v-card>
				...
				<v-card-text>
					...
					<RecordingList :showMenuButton="false" :openEntryOnClick="false" @ClickEntry="OnClickEntry"
						ref="list" />
					...
					<v-dialog v-model="newDialogueVisible" persistent scrollable :fullscreen="MobileDeviceWidth()">
						<v-card>
							...
							<v-divider></v-divider>
							<v-card-text>
								<RecordingEditor ref="editor" v-model="newDialogueObject" :showAppBar="false"
		...
		<v-input :messages="[]" ref="input" persistent-hint :hint="hint" :required="required" v-model="value"
			...
			<template v-slot:append>
				<v-menu bottom left>
					...
					<v-list dense>
						...
						<v-divider /> -->
						<v-list-item @click="SelectNewRecording()" :disabled="disabled || readonly">
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
							<v-list-item-content>
			...
			<div
				...
				<div v-if="!Recording && value">
					{{ value }} (can't find)
				</div>
				<div v-else-if="RecordingForId(value)" @click="SelectNewRecording()" :style="{
		flex: '1',
		display: 'flex',
		...
		color: disabled ? 'rgba(0, 0, 0, 0.38)' : 'inherit',
				...
				<div v-else @click="SelectNewRecording()" style="flex: 1; text-align: center;">
