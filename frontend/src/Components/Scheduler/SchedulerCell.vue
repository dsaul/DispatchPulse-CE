<template>
	<div class="base" :style="{
		...
		<v-dialog v-model="ModifyOthersDialogue" :fullscreen="MobileDeviceWidth()" scrollable persistent>
			<v-card>
				...
				<v-card-text>
					...
					<v-radio-group v-model="ModifyOthersDialogueChoice">
						<v-radio label="Move just this assignment and turn off project schedule syncing?"
							value="MoveJustThisAssignment">
						</v-radio>
						<v-radio label="Change the other assignments on this project to match this schedule as well."
				...
				<v-card-actions>
					<v-btn color="green darken-1" text @click="ModifyOthersDialogue = false">
						Cancel
					</v-btn>
					<v-spacer></v-spacer>

					<v-btn color="red darken-1" text @click="ModifyOthersDialogueCallback">
						Save
					</v-btn>
				</v-card-actions>
			</v-card>
		...
		<!---->
		<draggable v-if="PermAssignmentCanPush()" class="list-group" scroll-sensitivity="100" :force-fallback="true"
			:value="Events" group="people" @change="OnChange" :move="move1" :style="{
		//width: `${minimized ? cellWidthMinimizedPx : cellWidthPx}px`,
		flex: 1,
	}">
			<div class="list-group-item" v-for="(element, index) in Events" :key="element.id">
				<SchedulerEvent v-model="Events[index]" :cellWidthPx="cellWidthPx"
					:cellWidthMinimizedPx="cellWidthMinimizedPx" :agentId="agentId" />
			</div>
		</draggable>
