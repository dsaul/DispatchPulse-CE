<template>
	<div style="min-height: 60px; display:flex;background: white;">
		...
		<SchedulerCell v-for="(column, index) in columns" :key="index" :date="date" :agentId="agentId"
			:cellWidthPx="cellWidthPx" :cellWidthMinimizedPx="cellWidthMinimizedPx"
			:isUnscheduled="column.type == 'unscheduled'" :isFirstAM="column.type == 'am1'"
			:isSecondAM="column.type == 'am2'" :isFirstPM="column.type == 'pm1'" :isSecondPM="column.type == 'pm2'"
			:localTimeStart="column.localTimeStart" :localTimeEnd="column.localTimeEnd"
			:minimized="minimizedColumns[index]" />
		<div style="min-width: 300px;height: 1px;"></div>
