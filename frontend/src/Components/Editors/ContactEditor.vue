<template>
	<div>
		<v-app-bar v-if="showAppBar" color="#747389" dark fixed app clipped-right>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item @click="CSVDownloadContact(value)" :disabled="!PermCRMExportContactsCSV()">
						<v-list-item-icon>
					...
					<v-card flat>
						...
						<v-form ref="generalForm" autocomplete="newpassword">
							<v-container>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field v-model="Name" autocomplete="newpassword" label="Name"
											hint="The name of this contact." :rules="[
			ValidateRequiredField
		]" class="e2e-contact-editor-name" :disabled="connectionStatus != 'Connected'"
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-combobox label="Title" id="Title" v-model="Title" auto-select-first
											hint="This contact's title, for example: Journeyman Electrician"
											:items="$store.getters.SortedDeduplicatedContactTitles"
											class="e2e-contact-editor-title" :disabled="connectionStatus != 'Connected'"
											:readonly="!PermContactsCanPush()">
										</v-combobox>
									</v-col>
								</v-row>
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<CompanySelectField :isDialogue="isDialogue" v-model="CompanyId"
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-textarea label="Notes" hint="Additional information about this contact."
											v-model="Notes" class="e2e-contact-editor-notes"
											:disabled="connectionStatus != 'Connected'"
											:readonly="!PermContactsCanPush()"></v-textarea>
									</v-col>
								</v-row>
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
								...
								<PhoneNumberEditRowArrayAdapter v-model="PhoneNumbers"
									:disabled="connectionStatus != 'Connected'" :readonly="!PermContactsCanPush()" />
								...
								<AddressEditRowArrayAdapter v-model="Addresses"
									:disabled="connectionStatus != 'Connected'" :readonly="!PermContactsCanPush()" />
								...
								<EMailEditRowArrayAdapter v-model="EMails" :disabled="connectionStatus != 'Connected'"
									:readonly="!PermContactsCanPush()" />
				...
				<v-tab-item style="flex: 1;">
					...
					<ProjectList :showOnlyContactId="$route.params.id" :isDialogue="isDialogue" />
				</v-tab-item>
			</v-tabs-items>
		</div>
		...
		<v-footer v-if="showFooter" color="#747389" class="white--text" app inset>
			<v-row no-gutters>
				<v-btn :disabled="!value || connectionStatus != 'Connected' || !PermContactsCanDelete()" color="white"
					text rounded @click="DoDelete()">
...
export default class ContactEditor extends EditorBase {
	...
	protected DoDelete(): void {
		Dialogues.Open({
			name: 'DeleteContactDialogue', state: {
