	...
	<v-row>
		<v-col cols="4" sm="2" offset-sm="2">
			<v-combobox
				label="Label"
				auto-select-first
				hint="Home, Work, Jobsite, etc."
				v-model="Label"
				:items="[ 'Unspecified', 'Home', 'Work', 'Jobsite']"
				class="e2e-address-edit-row-label"
				:disabled="disabled"
				:readonly="readonly"
				>
			</v-combobox>
		...
		<v-col cols="8" sm="6">
			<v-textarea
				...
				:auto-grow="true"
				label="Address"
				hint="Enter a physical street address."
				class="e2e-address-edit-row-value"
				v-model="Value"
				:disabled="disabled"
				:readonly="readonly"
				>
				...
				<template v-slot:append-outer>
					<v-menu bottom left>
						...
						<v-list dense>
							<v-list-item
								@click="OpenInMaps()"
								:disabled="isDialogue || IsValueEmpty"
								>
								<v-list-item-icon>
