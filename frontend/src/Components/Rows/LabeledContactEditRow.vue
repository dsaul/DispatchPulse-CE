<template>
	<v-row>
		<v-col v-if="!hideLabel" cols="4" :sm="reducePadding ? 3 : 2" :offset-sm="reducePadding ? 0 : 2">
			<v-combobox v-model="Label" label="Label" auto-select-first hint="Relation to this project."
				:items="['Unspecified', 'Homeowner', 'Designer', 'Architect', 'Manager']"
				class="e2e-labeled-contact-edit-row-label" :disabled="disabled" :readonly="readonly">
			</v-combobox>
		</v-col>
		<v-col :cols="hideLabel ? 12 : 8" :sm="hideLabel ? 12 : (reducePadding ? 9 : 6)">
			<ContactSelectField v-model="Value" :isDialogue="isDialogue" class="e2e-labeled-contact-edit-row-value"
