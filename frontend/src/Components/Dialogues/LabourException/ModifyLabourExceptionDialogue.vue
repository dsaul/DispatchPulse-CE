<template>
	<v-dialog v-model="IsOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
		<v-card>
			<v-card-title>Modify Labour Exception</v-card-title>
			<v-divider></v-divider>
			<v-card-text>

				<LabourExceptionEditor ref="editor" v-model="ModelState" :showAppBar="false" :showFooter="false"
...
import LabourExceptionEditor from '@/Components/Editors/LabourExceptionEditor.vue';
...
export default class ModifyLabourExceptionDialogue extends DialogueBase {



	public $refs!: {
		editor: LabourExceptionEditor,
	};
	...
	constructor() {
		super();
		this.ModelState = LabourSubtypeException.GetEmpty();
	}



	get DialogueName(): string {
		return 'ModifyLabourExceptionDialogue';
	}
	...
	protected Save(): void {
		...
		if (this.$refs.editor.IsValidated()) {
			const state = this.ModelState as ILabourSubtypeException;
			if (state.id) {
				...
				LabourSubtypeException.UpdateIds(payload);
