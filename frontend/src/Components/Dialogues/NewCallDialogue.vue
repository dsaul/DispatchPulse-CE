<template>
	<v-dialog v-model="IsOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
		<v-card>
			...
			<v-divider></v-divider>
			<v-card-text>
				<v-form autocomplete="newpassword" ref="form">
					...
					<v-stepper v-model="currentStep" vertical style="-webkit-box-shadow: inherit; box-shadow: inherit;">
						...
						<v-stepper-content step="1">
							...
							<LabeledContactEditRowArrayAdapter v-model="Contacts" :reducePadding="true"
								class="e2e-contacts-array" />

							<!-- Used solely to display the validation errors for the above array. -->
							<v-input v-model="Contacts" :rules="[
		this.EnsureContactArrayHasAtLeastOneContact
	]" />

						</v-stepper-content>

						<v-stepper-step :complete="currentStep > 2" editable step="2" class="e2e-step-project">
							Project
						...
						<v-stepper-content step="2">
							...
							<ProjectSelectField v-model="ProjectId" :required="true" :rules="[
		this.EnsureProjectFieldIsFilled
	]" class="e2e-new-call-dialogue-project-select-field" />

						</v-stepper-content>

						<v-stepper-step :complete="currentStep > 3" editable step="3" class="e2e-step-assignments">
							Assignments
						...
						<v-stepper-content step="3">
							<p style="margin-top: 40px;">
								<v-slider v-model="NumberOfAssignments" :max="100" :min="1" step="1"
									label="Assignment Count" thumb-label ticks :rules="[ValidateRequiredField]">
									<template v-slot:append>
										<v-text-field v-model="NumberOfAssignments" class="mt-0 pt-0" hide-details
											single-line type="number" step="1" min="1" max="100" style="width: 60px">
										</v-text-field>
									</template>
								</v-slider>
							</p>
						...
						<v-stepper-step step="4" editable class="e2e-step-additional-information">
							Additional Information
						</v-stepper-step>
						<v-stepper-content step="4">
							<v-textarea v-model="WorkRequested" label="Work Requested"
								class="e2e-new-call-dialogue-work-requested" auto-grow>
							</v-textarea>
							<v-textarea v-model="InternalComments"
								label="Internal Comments (Not to be Shared With Clients)"
								class="e2e-new-call-dialogue-internal-comments" auto-grow>
							</v-textarea>
						</v-stepper-content>
					</v-stepper>
			...
			<v-card-actions>
				...
				<v-btn color="blue darken-1" text @click="Next()" class="e2e-new-call-dialogue-next-button">Next</v-btn>
			</v-card-actions>
		</v-card>
	</v-dialog>
</template>
