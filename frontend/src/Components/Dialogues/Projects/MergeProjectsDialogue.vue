	...
	<v-dialog v-model="IsOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
		<v-card>
			...
			<v-card-text>
				...
				<v-alert outlined type="warning" elevation="0" style="margin-top: 20px;">
				...
				<v-stepper v-model="currentStep" vertical style="-webkit-box-shadow: inherit; box-shadow: inherit;">
					<v-stepper-step :complete="ProjectIDsValidates" editable step="1" class="e2e-merge-dialogue-step-1">
						Select the projects to merge.
						<small>Select as many as required.</small>
					</v-stepper-step>

					<v-stepper-content step="1">
						<ProjectSelectFieldArrayAdapter v-model="ProjectIDs" :showDetails="false" :isDialogue="true"
							class="e2e-merge-projects-select-field-adapter" />
					</v-stepper-content>

					<v-stepper-step :complete="SelectedName != NULL_SENTINEL" editable step="2"
					...
					<v-stepper-content step="2">
						...
						<v-alert outlined type="info" elevation="0" style="" v-if="!AreThereProjectsSelected">
							No projects selected above.
						</v-alert>
						<v-alert outlined type="warning" elevation="0" style=""
							v-if="AreThereProjectsSelected && ProjectNames.length === 0">
							No names on any of the selected projects.
						</v-alert>

						<v-radio-group v-if="AreThereProjectsSelected" v-model="SelectedName">
							<v-radio label="No name selected." :value="NULL_SENTINEL">
							</v-radio>
							<v-radio v-for="(name, index) of ProjectNames" :key="index" :label="name" :value="name">
							</v-radio>
						</v-radio-group>
					...
					<v-stepper-step :complete="SelectedParent != NULL_SENTINEL" editable step="3"
					...
					<v-stepper-content step="3">
						...
						<v-radio-group v-if="AreThereProjectsSelected" v-model="SelectedParent">
							<v-radio label="No parent project selected." :value="NULL_SENTINEL">
							</v-radio>
							<v-radio v-for="(project, index) of ParentProjects" :key="index"
								:label="ProjectCombinedDescriptionForId(project.id)" :value="project.id">
							</v-radio>
						</v-radio-group>
					...
					<v-stepper-step :complete="SelectedStatus != NULL_SENTINEL" editable step="4"
					...
					<v-stepper-content step="4">
						...
						<v-radio-group v-if="AreThereProjectsSelected" v-model="SelectedStatus">
							<v-radio label="No status selected." :value="NULL_SENTINEL">
							</v-radio>
							<v-radio v-for="(status, index) of Statuses" :key="index" :label="status.name"
								:value="status.id">
							</v-radio>
						</v-radio-group>
					...
					<v-stepper-step step="5" editable :complete="SelectedSchedule != NULL_SENTINEL"
					...
					<v-stepper-content step="5">
						...
						<v-radio-group v-if="AreThereProjectsSelected" v-model="SelectedSchedule">
							<v-radio label="No schedule selected." :value="NULL_SENTINEL">
							</v-radio>
							<v-radio v-for="(schedule, index) of Schedules" :key="index"
					...
					<v-stepper-step step="6" editable :complete="currentStep > 6" class="e2e-merge-dialogue-step-6">
						Project Settings
					</v-stepper-step>
					<v-stepper-content step="6">
						...
						<v-switch v-model="ComputedForceLabourAsExtra" :label="`Force labour to be extra.`"
							:disabled="true">
						</v-switch>

						<v-switch v-model="ComputedForceAssignmentsToUseProjectSchedule"
							:label="`Force assignments to use project schedule.`" :disabled="true">
						</v-switch>
			...
			<v-card-actions>
				<v-spacer />
				<v-btn color="blue darken-1" text @click="ResetAndClose()">
					Close
				</v-btn>
				<v-btn color="blue darken-1" text @click="Next()" class="e2e-merge-projects-dialogue-next-button">
					Next
				</v-btn>
			</v-card-actions>
		</v-card>
