<template>
	<v-dialog v-model="IsOpen" persistent scrollable :fullscreen="MobileDeviceWidth()">
		<v-card>
			<v-card-title>Modify Product</v-card-title>
			<v-divider></v-divider>
			<v-card-text>

				<ProductEditor ref="editor" v-model="ModelState" :showAppBar="false" :showFooter="false"
...
export default class ModifyProductDialogue extends DialogueBase {
	...
	constructor() {
		super();
		this.ModelState = Product.GetEmpty();
	}



	get DialogueName(): string {
		return 'ModifyProductDialogue';
	}
	...
	protected Save(): void {
		...
		if (this.$refs.editor.IsValidated()) {
			const state = this.ModelState as IProduct;
			if (state.id) {
