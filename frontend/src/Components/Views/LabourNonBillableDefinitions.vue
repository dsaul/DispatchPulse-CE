<template>
	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right >
			...
			<v-toolbar-title class="white--text">Non Billable Labour Definitions</v-toolbar-title>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item
						@click="CSVDownloadLabourSubtypeNonBillable()"
						:disabled="!PermCRMExportLabourSubtypeNonBillableCSV()"
						>
						<v-list-item-icon>
			...
			<template v-slot:extension>
				...
				>
					...
					<v-tab @click="$router.replace({query: { ...$route.query, tab: 'Types'}}).catch(((e) => {}));">
						Types
					</v-tab>
				</v-tabs>
			</template>
		...
		<v-footer color="#747389" class="white--text" app inset>
			<v-row
				...
				<AddMenuButton
					...
					<v-list-item
						@click="AddContact()"
						class="e2e-labour-non-billable-add-definition"
						:disabled="connectionStatus != 'Connected' || !PermLabourSubtypeNonBillableCanPush()"
						>
						<v-list-item-icon>
							<v-icon>add</v-icon>
						</v-list-item-icon>
...
import LabourNonBillableDataTable from '@/Components/DataTables/LabourNonBillableDataTable.vue';
import CSVDownloadLabourSubtypeNonBillable from '@/Data/CRM/LabourSubtypeNonBillable/CSVDownloadLabourSubtypeNonBillable';
...
export default class LabourNonBillableSettings extends ViewBase {
	
	public $refs!: {
		dataTable: LabourNonBillableDataTable,
	};
	...
	protected CSVDownloadLabourSubtypeNonBillable = CSVDownloadLabourSubtypeNonBillable;
	...
	protected PermCRMExportLabourSubtypeNonBillableCSV = LabourSubtypeNonBillable.PermCRMExportLabourSubtypeNonBillableCSV;
	...
	protected AddContact(): void {
		Dialogues.Open({ name: 'ModifyLabourNonBillableDialogue', state: null});
	}
