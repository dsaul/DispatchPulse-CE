	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item @click="CSVDownloadCompanies()" :disabled="!PermCRMExportCompaniesCSV()">
						<v-list-item-icon>
			...
			<template v-slot:extension>
				<v-tabs v-model="tab" background-color="transparent" align-with-title show-arrows>
					...
					<v-tab @click="$router.replace({ query: { ...$route.query, tab: 'Companies' } }).catch(((e) => { }));"
		...
		<v-tabs-items v-model="tab" style="background: transparent;">
			<v-tab-item style="flex: 1;">
				<CompanyList ref="companyList" :disabled="connectionStatus != 'Connected'" />
			</v-tab-item>
		</v-tabs-items>
		...
		<v-footer color="#747389" class="white--text" app inset>
			<v-row no-gutters>
				...
				<AddMenuButton :disabled="connectionStatus != 'Connected'">
					<v-list-item @click="AddCompany()" class="e2e-add-menu-add-company"
