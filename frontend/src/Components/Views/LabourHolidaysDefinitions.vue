<template>
	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right >
			...
			<v-toolbar-title class="white--text">Labour Holiday Definitions</v-toolbar-title>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item
						@click="CSVDownloadLabourSubtypeHolidays()"
						:disabled="!PermCRMExportLabourSubtypeHolidayCSV()"
						>
						<v-list-item-icon>
			...
			<template v-slot:extension>
				...
				>
					...
					<v-tab @click="$router.replace({query: { ...$route.query, tab: 'Holidays'}}).catch(((e) => {}));">
						Holidays
					</v-tab>
				</v-tabs>
			</template>
		...
		<v-footer color="#747389" class="white--text" app inset>
			<v-row
				...
				<AddMenuButton
					...
					<v-list-item
						@click="AddContact()"
						class="e2e-labour-holidays-definitions-add-definition"
						:disabled="connectionStatus != 'Connected' || !PermLabourSubtypeHolidayCanPush()"
						>
						<v-list-item-icon>
							<v-icon>add</v-icon>
						</v-list-item-icon>
...
import LabourHolidayDataTable from '@/Components/DataTables/LabourHolidayDataTable.vue';
import CSVDownloadLabourSubtypeHolidays from '@/Data/CRM/LabourSubtypeHoliday/CSVDownloadLabourSubtypeHolidays';
...
export default class LabourHolidaysSettings extends ViewBase {
	
	public $refs!: {
		dataTable: LabourHolidayDataTable,
	};
	...
	public breadcrumbs = [
		...
		{
			text: 'Labour Holiday Definitions',
			disabled: true,
	...
	protected CSVDownloadLabourSubtypeHolidays = CSVDownloadLabourSubtypeHolidays;
	...
	protected PermCRMExportLabourSubtypeHolidayCSV = LabourSubtypeHoliday.PermCRMExportLabourSubtypeHolidayCSV;
	...
	protected AddContact(): void {
		Dialogues.Open({ name: 'ModifyLabourHolidayDialogue', state: null});
	}
