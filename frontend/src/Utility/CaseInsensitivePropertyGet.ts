...
export default (object: Record<string, any>, key: string): any => {
	...
	if (!key.toLowerCase) {
		console.error('CaseInsensitivePropertyGet !key.toLowerCase, supplied key doesn\'t appear to be a string?', key);
		return null;
	}
	...
	const foundKey: string | null = Object.keys(object).find((o) => {
		
		if (!o.toLowerCase) {
			console.warn('iterating keys, found one that doesn\'t appear to be a string, saying it doesn\'t match', o);
			return false;
		}
