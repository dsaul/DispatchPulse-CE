...
export default (start: string, end: string): DurationObjectUnits => {
	...
	const res = endLocal
		.diff(startLocal, ["hours", "minutes", "seconds"])
		.toObject();

	if (
		!res.hasOwnProperty("hours") ||
		!res.hasOwnProperty("minutes") ||
		!res.hasOwnProperty("seconds")
	) {
		...
		return {
			...
			minutes: -9999,
			seconds: -9999
		} as DurationObjectUnits; // Return something obviously wrong.
	}
