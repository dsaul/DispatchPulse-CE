...
export class RPCMethod {
	public static Register<T extends RPCMethod>(method: T): T {
		...
		if (!callbackMethodName || IsNullOrEmpty(callbackMethodName)) {
			console.error(
				"!callbackMethodName || IsNullOrEmpty(callbackMethodName)"
			);
			return method;
		}

		SignalRConnection.Connection.on(callbackMethodName, params =>
			method.Recieve(params)
		);
	...
	public Send(payload: IIdempotencyRequest): IRoundTripRequest {
		...
		if (!serverMethodName || IsNullOrEmpty(serverMethodName)) {
			ret.outboundRequestPromise = Promise.reject(
				new Error("No Server Method Name!")
			);
		...
		ret.outboundRequestPromise.catch(err => {
			console.error(
				`${serverMethodName} Network Error`,
				SignalRConnection.Connection?.state,
				err.toString()
			);
