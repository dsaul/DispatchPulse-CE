...
class SignalRConnection {
	...
	constructor() {
		...
		window
			...
			.then(data => {
						...
						nextRetryDelayInMilliseconds: retryContext => {
							if (
								...
								Notifications.AddNotification({
									severity: "warning",
									message: `Disconnected. Waiting ${Math.round(
										(waitMilis / 1000) * 10
								...
								console.error(
									`We've tried reconnecting for a day. Reload the app to try connecting again.`
								);
				...
				this.connection.on("CreateBackupTaskCB", (params: any) =>
					this.CreateBackupTaskCB(params)
				);
		...
		const promise = this.connection
			...
			.catch((err: Error) => {
				...
				if (err && err.toString() === "Error") {
					errorText =
						"Unable to connect to API. Retrying in 5 seconds...";
				} else if (err) {
	...
	public CreateBackupTask(payload: ICreateBackupTask): Promise<any> {
		...
		return this.connection
			.invoke("CreateBackupTask", payload)
			.then(() => {
				//
			})
			.catch((err: Error) => {
				return console.error(
					"CreateBackupTask Network Error",
