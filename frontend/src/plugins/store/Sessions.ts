			...
			SignalRConnection.Ready(() => {
				...
				if (!GetDemoMode()) {
					...
					if (billingContactsRTR.completeRequestPromise) {
						billingContactsRTR.completeRequestPromise.catch((e: Error) => {
							console.error('SetSession>RequestBillingContactsForCurrentSession error', e);
					...
					if (billingSessionsRTR.completeRequestPromise) {
						billingSessionsRTR.completeRequestPromise.catch((e: Error) => {
							console.error('SetSession>RequestBillingSessionsForCurrentSession error', e);
