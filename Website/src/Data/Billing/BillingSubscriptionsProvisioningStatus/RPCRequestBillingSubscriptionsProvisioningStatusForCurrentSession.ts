...
export interface IRequestBillingSubscriptionsProvisioningStatusForCurrentSessionCB extends IIdempotencyResponse {
...
export class RPCRequestBillingSubscriptionsProvisioningStatusForCurrentSession extends RPCMethod {
	public Send(payload: IRequestBillingSubscriptionsProvisioningStatusForCurrentSessionPayload): IRoundTripRequest {
		...
		if (!payload.billingSubscriptionsProvisioningStatus) {
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(`Error requesting subscriptions provisioning status #2.`));
			}
			return false;
		}
	
		// Default action
		const newPayload: Record<string, IBillingSubscriptionsProvisioningStatus> = {};
		
		for (const e of payload.billingSubscriptionsProvisioningStatus) {
			newPayload[e.uuid] = e;
		}
		
		store.commit('UpdateBillingSubscriptionsProvisioningStatusRemote', newPayload);
