...
export class Material {
	// RPC Methods
	public static RequestMaterials = RPCMethod.Register<RPCRequestMaterials>(new RPCRequestMaterials());
	public static DeleteMaterials = RPCMethod.Register<RPCDeleteMaterials>(new RPCDeleteMaterials());
	public static PushMaterials = RPCMethod.Register<RPCPushMaterials>(new RPCPushMaterials());
	...
	public static GetEmpty(): IMaterial {
		...
		const ret: IMaterial = {
			...
			json: {
				...
				dateUsedISO8601: DateTime.local().toUTC().toISO(),
				projectId: null,
				quantity: null,
				quantityUnit: null,
				productId: null,
	...
	public static ForId(id: string | null): IMaterial | null {
		...
		const statuses = store.state.Database.materials as Record<string, IMaterial>;
	...
	public static PermCRMReportMaterialsPDF(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.report.materials-pdf') !== -1;
		//console.log(perms, ret);
		return ret;
	...
	public static PermCRMExportMaterialsCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.export.materials-csv') !== -1;
		//console.log(perms, ret);
		return ret;
