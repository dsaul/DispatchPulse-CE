...
export class Labour {
	// RPC Methods
	public static RequestLabour = RPCMethod.Register<RPCRequestLabour>(new RPCRequestLabour());
	public static DeleteLabour = RPCMethod.Register<RPCDeleteLabour>(new RPCDeleteLabour());
	public static PushLabour = RPCMethod.Register<RPCPushLabour>(new RPCPushLabour());
	...
	public static ForAssignmentIds(projectIds: string[]): ILabour[] {
		...
		const filtered = _.filter(all, (o: ILabour) => {
			...
			return !!_.find(projectIds, (suppliedId: string) => suppliedId === assignmentId);
		});
	...
	public static DescriptionForId(id: string | null): string | null {
		...
		if (type.json.isBillable) {
			...
			if (!exceptionTypeId) {
				...
				const exceptionType = LabourSubtypeException.ForId(exceptionTypeId);
				if (!exceptionType) {
			...
			if (!nonBillableTypeId) {
				...
				const nonBillableType = LabourSubtypeNonBillable.ForId(nonBillableTypeId);
				if (!nonBillableType) {
	...
	public static PermCRMReportLabourPDF(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.report.labour-pdf') !== -1;
		//console.log(perms, ret);
		return ret;
	...
	public static PermCRMExportLabourCSV(): boolean {
		...
		const perms = BillingPermissionsBool.AllForBillingContactId();
		const ret = 
			perms.indexOf('crm.export.labour-csv') !== -1;
		//console.log(perms, ret);
		return ret;
