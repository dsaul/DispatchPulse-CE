...
export default class ListBase extends Vue {
	...
	@Prop({ default: 'There are no entries to show.' }) public readonly emptyMessage!: string;
	...
	public get PageRows(): ICRMTable[] {
		...
		const end = (this.rowsPerPage * (this.CurrentPage - 1)) + this.rowsPerPage;
	...
	protected RowFilter(o: ICRMTable, key: string): boolean {
		console.error('RowFilter is not subclassed! returning true for everything!');
		return true;
	}
	
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	protected RowSortBy(o: string | number | boolean | symbol | ICRMTable): string | number | boolean | symbol | ICRMTable {
	...
	protected GetDeleteEntryDialogueName(): string {
		console.error('GetDeleteEntryDialogueName is not subclassed! returning ""!');
		return '';
	}
