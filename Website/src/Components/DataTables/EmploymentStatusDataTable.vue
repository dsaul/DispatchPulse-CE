<template>
	<v-card flat>
		...
		<v-text-field
			...
			class="mx-4 e2e-employment-status-data-table-filter"
		...
		<v-data-table
			v-if="PermEmploymentStatusCanRequest()"
			:headers="headers"
			:items="AllEmploymentStatuses"
			:search="searchString"
			item-key="name"
				...
				<v-menu bottom left>
					...
					<v-list dense>
						<v-list-item
							...
							:disabled="disabled || !PermEmploymentStatusCanPush()"
							>
							<v-list-item-icon>
								<v-icon>edit</v-icon>
							</v-list-item-icon>
