	...
	<div class="outerdiv" style="border-bottom: ">
		...
		<v-input
			...
			<template v-slot:append>
				<v-menu bottom left>
					...
					<v-list dense>
						...
						<v-divider />
						<v-list-item
							@click="SelectNewOnCallAutoAttendant()"
			...
			<div style="margin-top: 5px; margin-bottom: 5px; width: 100%; cursor: text; display: flex; align-items: stretch;">
				<div
					v-if="!OnCallAutoAttendant && value"
					>
					{{value}} (can't find)
				</div>
				<div 
					v-else-if="OnCallAutoAttendantForId(value)"
					@click="SelectNewOnCallAutoAttendant()"
					:style="{
						flex: '1',
						display: 'flex',
				...
				<div
					v-if="OnCallAutoAttendantForId(value) && !isDialogue"
					...
					<v-tooltip
						...
						<template v-slot:activator="{ on }">
							<v-chip
								:to="`/section/on-call/${value}?tab=General`"
								color="primary"
								label
								outlined
								style="margin: 4px;"
