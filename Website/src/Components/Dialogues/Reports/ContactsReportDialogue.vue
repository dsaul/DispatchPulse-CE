<template>
	<v-dialog
		...
		>
		<v-card>
			<v-card-title>Contacts Report</v-card-title>
			<v-divider></v-divider>
			<v-card-text >
						...
						<v-container>
							...
							<v-row v-if="_RenderingActive == false && _RenderingComplete == false">
								<v-col
									...
									>
									<v-switch
										v-model="AllLoadedContacts"
										label="All Loaded Contacts"
										style="margin-top:0px;"
										>
									</v-switch>
								</v-col>
							</v-row>
							<v-row v-if="!AllLoadedContacts && _RenderingActive == false && _RenderingComplete == false">
								<v-col
									cols="12"
									sm="8"
									offset-sm="2"
...
import { IRunReportContactsCB } from '@/Data/Reports/RPCRunReportContacts';
...
interface ContactsReportState {
	allLoadedContacts: boolean;
	specificContacts: ILabeledContactId[];
...
export default class ContactsReportDialogue extends DialogueBase {
	
	public static GenerateEmpty(): ContactsReportState {
	...
	constructor() {
		super();
		this.ModelState = ContactsReportDialogue.GenerateEmpty();
	}
	
	
	
	get DialogueName(): string {
		return 'ContactsReportDialogue';
	}
	...
	public get _RenderingActive(): boolean {
		...
		return (this.ModelState as ContactsReportState)._renderingActive;
	}
	
	public set _RenderingActive(flag: boolean) {
		const state = this.ModelState as ContactsReportState;
		if (!state) {
	...
	public get _RenderingProgressMessage(): string {
		...
		return (this.ModelState as ContactsReportState)._renderingProgressMessage;
	}
	...
	public get _ErrorMessage(): string {
		...
		return (this.ModelState as ContactsReportState)._errorMessage;
	}
	...
	public get _ShowProgress(): boolean {
		...
		return (this.ModelState as ContactsReportState)._showProgress;
	}
	...
	public get _RenderingComplete(): boolean {
		...
		return (this.ModelState as ContactsReportState)._renderingComplete;
	}
	...
	public get _DownloadLink(): string | null {
		...
		return (this.ModelState as ContactsReportState)._downloadLink;
	}
	...
	public get AllLoadedContacts(): boolean {
		if (!this.ModelState) {
			console.warn('Attempted get on null ModelState');
			return false;
		}
		return (this.ModelState as ContactsReportState).allLoadedContacts;
	}
	
	public set AllLoadedContacts(flag: boolean) {
		const state = this.ModelState as ContactsReportState;
		if (!state) {
			console.warn('Attempted set on null ModelState');
			return;
		}
		state.allLoadedContacts = flag;
		this.ModelState = state;
	}

	get SpecificContacts(): ILabeledContactId[] {
		if (!this.ModelState) {
			console.warn('Attempted get on null ModelState');
			return [];
		}
		const state = this.ModelState;
		return state.specificContacts;
		
	}
	
	set SpecificContacts(val: ILabeledContactId[]) {
		const state = this.ModelState;
		if (!state) {
			console.warn('Attempted set on null ModelState');
			return;
		}
		state.specificContacts = val;
		this.ModelState = state;
	}
	...
	protected Run(): void {
		...
		do {
			...
			const specificIds = this.SpecificContacts;
			const filtered = [];
			if (false === this.AllLoadedContacts) {
			...
			if (false === this.AllLoadedContacts && filtered.length === 0) {
				Notifications.AddNotification({
					severity: 'error',
			...
			const rtr = Reports.RunReportContacts.Send({
				sessionId: BillingSessions.CurrentSessionId(),
				runOnAllContacts: this.AllLoadedContacts,
				contactIds: filtered,
			});
			...
			if (rtr.completeRequestPromise) {
				...
				rtr.completeRequestPromise.then((payload: IRunReportContactsCB) => {
					console.log('RunReportContacts returned', payload);
