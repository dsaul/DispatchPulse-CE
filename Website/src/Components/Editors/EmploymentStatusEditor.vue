<template>
	<div>
		...
		<v-app-bar
			...
			<v-toolbar-title class="white--text">Employment Status</v-toolbar-title>
			...
			<v-container>
				<v-row>
					<v-col cols="12" sm="8" offset-sm="2">
						<div class="title">Employment Status Not Found</div>
					</v-col>
				</v-row>
				<v-row>
					<v-col cols="12" sm="8" offset-sm="2">
						...
						<ul>
							...
							<li>The employment status no longer exists and this is an old bookmark.</li>
							<li>Someone deleted the employment status while you were opening it.</li>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field
											...
											autocomplete="newpassword"
											label="Name"
											hint="Name of the Employment Status"
											:rules="[ ValidateRequiredField ]"
											class="e2e-employment-status-editor-name"
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-switch
											...
											label="Should be Listed in Scheduler"
											v-model="ShouldBeListedInScheduler"
											>
										</v-switch>
									</v-col>
								</v-row>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field
											...
											readonly="readonly"
											label="Unique ID"
											hint="The id of this employment status."
											>
										</v-text-field>
									</v-col>
								</v-row>
		...
		<v-footer
			...
			<v-row
				...
				<v-btn
					...
					text
					rounded
					@click="DialoguesOpen({ name: 'DeleteEmploymentStatusDialogue', state: {
...
import { IEmploymentStatus } from '@/Data/CRM/EmploymentStatus/EmploymentStatus';
...
export default class EmploymentStatusEditor extends EditorBase {

	@Prop({ default: null }) declare public readonly value: IEmploymentStatus | null;
	...
	protected get ShouldBeListedInScheduler(): boolean {
		...
		return this.value.json.shouldBeListedInScheduler;
	}
	
	protected set ShouldBeListedInScheduler(val: boolean) {
		...
		this.value.json.shouldBeListedInScheduler = val;
		this.SignalChanged();
	}
