<template>
	<div>
		...
		<v-app-bar
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item
						@click="CSVDownloadAgent(value)"
						:disabled="!PermCRMExportAgentsCSV()"
						>
						<v-list-item-icon>
		...
		<div v-else>
			...
			<v-tabs-items v-model="tab" style="background: transparent;">
				<v-tab-item style="flex: 1;">
					<AssignmentsList
						:showOnlyAgentId="$route.params.id"
						:focusIsProject="true"
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field
											...
											hint="The name of this agent"
											class="e2e-agent-editor-name"
											:disabled="connectionStatus != 'Connected'"
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field
											...
											autocomplete="newpassword" 
											label="Title" 
											hint="The title of this agent, for example: Electrician"
											required
											:rules="[
													OptionalButMustBeMoreThan4Characters
												]"
											class="e2e-agent-editor-title"
											:disabled="connectionStatus != 'Connected'"
											:readonly="!PermAgentsCanPush()"
											>
										</v-text-field>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field
											...
											autocomplete="newpassword" 
											label="SMS/Text Phone Number" 
											hint="The number to send SMSes to."
											type="tel"
											required
											:rules="[
												OptionalPhoneNumber
												]"
											class="e2e-agent-editor-sms-number"
											:disabled="connectionStatus != 'Connected'"
											:readonly="!PermAgentsCanPush()"
											>
										</v-text-field>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-select
											...
											:items="EmploymentStatusAll()"
											item-text="json.name"
											item-value="id"
											v-model="EmploymentStatusId"
											class="e2e-agent-editor-employment-status"
											:disabled="connectionStatus != 'Connected'"
											:readonly="!PermAgentsCanPush()"
											>
										</v-select>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-text-field
											...
											prefix="$"
											suffix="per hour"
											hint="Enter the wage this agent is paid per hour."
											v-model="HourlyWage"
		...
		<v-footer
			...
			inset>
			<v-btn
				:disabled="!value || connectionStatus != 'Connected' || !PermAgentsCanDelete()"
				color="white"
				text
				rounded
				@click="DialoguesOpen({ name: 'DeleteAgentDialogue', state: {
