<template>
	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right >
			...
			<v-toolbar-title class="white--text">All Materials</v-toolbar-title>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item
						@click="CSVDownloadMaterials()"
						:disabled="!PermCRMExportMaterialsCSV()"
						>
						<v-list-item-icon>
			...
			<template v-slot:extension>
				<v-tabs
					...
					<v-tab @click="$router.replace({query: { ...$route.query, tab: 'Materials'}}).catch(((e) => {}));">
						Materials
					</v-tab>
				</v-tabs>
			</template>
		...
		<v-footer color="#747389" class="white--text" app inset>
			<v-row
				...
				<AddMenuButton
					...
					<v-list-item
						@click="AddMaterial()"
						class="e2e-materials-index-add-material"
...
import MaterialsDataTable from '@/Components/DataTables/MaterialsDataTable.vue';
import CommitSessionGlobalButton from '@/Components/Buttons/CommitSessionGlobalButton.vue';
import { Component } from 'vue-property-decorator';
import IsNullOrEmpty from '@/Utility/IsNullOrEmpty';
import SignalRConnection from '@/RPC/SignalRConnection';
import CSVDownloadMaterials from '@/Data/CRM/Material/CSVDownloadMaterials';
...
export default class MaterialsIndex extends ViewBase {
	
	public $refs!: {
		dataTable: MaterialsDataTable,
	};
	...
	public breadcrumbs = [
		...
		{
			text: 'All Material Entries',
			disabled: true,
			to: '/section/material-entries/index',
		},
	];
	
	protected CSVDownloadMaterials = CSVDownloadMaterials;
	...
	protected PermCRMExportMaterialsCSV = Material.PermCRMExportMaterialsCSV;
	...
	protected AddMaterial(): void {
		Dialogues.Open({ name: 'ModifyMaterialDialogue', state: null});
	}
