<template>
	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right >
			...
			<v-toolbar-title class="white--text">Project Status Definitions</v-toolbar-title>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item
						@click="CSVDownloadProjectStatus()"
						:disabled="!PermCRMExportProjectStatusCSV()"
						>
						<v-list-item-icon>
			...
			<template v-slot:extension>
				<v-tabs
					...
					<v-tab @click="$router.replace({query: { ...$route.query, tab: 'Statuses'}}).catch(((e) => {}));">
						Statuses
					</v-tab>
				</v-tabs>
			</template>
		...
		<v-footer color="#747389" class="white--text" app inset>
			<v-row
				...
				<AddMenuButton
					...
					<v-list-item
						@click="AddDefinition()"
						class="e2e-project-status-definitions-add-project-status"
						:disabled="connectionStatus != 'Connected' || !PermProjectStatusCanPush()"
						>
						<v-list-item-icon>
							<v-icon>add</v-icon>
						</v-list-item-icon>
...
import ProjectStatusDataTable from '@/Components/DataTables/ProjectStatusDataTable.vue';
import CSVDownloadProjectStatus from '@/Data/CRM/ProjectStatus/CSVDownloadProjectStatus';
...
export default class ProjectStatusDefinitions extends ViewBase {
	
	public $refs!: {
		dataTable: ProjectStatusDataTable,
	};
	...
	public breadcrumbs = [
		...
		{
			text: 'Project Status Definitions',
			disabled: true,
	...
	protected CSVDownloadProjectStatus = CSVDownloadProjectStatus;
	protected PermCRMExportProjectStatusCSV = ProjectStatus.PermCRMExportProjectStatusCSV;
	...
	protected AddDefinition(): void {
		Dialogues.Open({ name: 'ModifyProjectStatusDialogue', state: null});
	}
