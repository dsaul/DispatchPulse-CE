...
export interface IPerformRegisterAdditionalUsersCB extends IIdempotencyResponse {
...
export class RPCPerformRegisterAdditionalUsers extends RPCMethod {
	...
	public RecieveDefaultAction(rtr: IRoundTripRequest, payload: IPerformRegisterAdditionalUsersCB): boolean {
		
		if (payload.isError) {
		
			if (rtr && rtr._completeRequestPromiseReject) {
				rtr._completeRequestPromiseReject(
					new Error(`Error registering additional users: ${payload.errorMessage}`));
			}
			return false;
		}
