	...
	<div>
							...
							<v-container>
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										...
										<v-text-field
											v-model="MessageLeftAt"
											label="Message Left At"
											:readonly="true"
											>
										</v-text-field>
										<v-text-field
								...
								<v-row>
									<v-col cols="12" sm="8" offset-sm="2">
										<v-btn
											class="ma-2"
											color=""
											:disabled="!value || IsHandled"
											@click="MarkAsHandled"
											:loading="MarkAsHandledLoading"
											>
											Mark as Handled
										</v-btn>
									</v-col>
										...
										<v-timeline
											...
											<v-timeline-item
												...
												<div v-if="entry.type === 'text'">
													<v-card-title style="margin: 0px; padding: 0px; word-break: normal; font-size: 1rem">{{entry.description}}</v-card-title>
													<v-card-subtitle style="margin: 0px; padding: 0px; word-break: normal;">{{ISO8601ToLocalDatetimeSuperSpecific(entry.timestampISO8601)}}</v-card-subtitle>
		...
		<v-footer
			...
			<v-row
				...
				>
				<v-btn
					:disabled="!value || connectionStatus != 'Connected' || !PermVoicemailsCanDelete()"
					color="white"
					text
					rounded
					@click="OpenDeleteVoicemail()"
					>
