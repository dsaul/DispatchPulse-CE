<template>
	<v-dialog
		...
		>
		<v-card>
			<v-card-title>Modify Contact</v-card-title>
			<v-divider></v-divider>
			<v-card-text >
...
export default class ModifyContactDialogue extends DialogueBase {
	...
	constructor() {
		super();
		this.ModelState = Contact.GetEmpty();
	}
	...
	protected Save(): void {
		...
		if (this.$refs.editor.IsValidated()) {
			const state = this.ModelState as IContact;
			if (state.id) {
				...
				this.$router.push(`/section/contacts/${state.id}?tab=General`).catch(((e: Error) => { }));// eslint-disable-line
