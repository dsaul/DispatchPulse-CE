<template>
	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right >
			...
			<v-toolbar-title class="white--text">Man Hours Definitions</v-toolbar-title>
			...
			<v-menu bottom left offset-y>
				...
				<v-list dense>
					...
					<v-list-item
						@click="CSVDownloadManHours()"
						:disabled="!PermCRMExportManHoursCSV()"
						>
						<v-list-item-icon>
			...
			<template v-slot:extension>
				...
				>
					...
					<v-tab @click="$router.replace({query: { ...$route.query, tab: 'Man Hours'}}).catch(((e) => {}));">
						Man Hours
					</v-tab>
				</v-tabs>
			</template>
		...
		<v-footer color="#747389" class="white--text" app inset>
			<v-row
				...
				<AddMenuButton
					...
					<v-list-item
						@click="AddDefinition()"
						class="e2e-man-hours-definitions-add-definition"
						:disabled="connectionStatus != 'Connected' || !PermEstimatingManHoursCanPush()"
						>
						<v-list-item-icon>
							<v-icon>add</v-icon>
						</v-list-item-icon>
...
import ManHoursDataTable from '@/Components/DataTables/ManHoursDataTable.vue';
import CSVDownloadManHours from '@/Data/CRM/EstimatingManHours/CSVDownloadManHours';
import ViewBase from '@/Components/Views/ViewBase';
import SignalRConnection from '@/RPC/SignalRConnection';
import ReloadButton from '@/Components/Buttons/ReloadButton.vue';
import { EstimatingManHours } from '@/Data/CRM/EstimatingManHours/EstimatingManHours';
...
export default class ManHoursDefinitions extends ViewBase {
	
	public $refs!: {
		dataTable: ManHoursDataTable,
	};
	...
	protected CSVDownloadManHours = CSVDownloadManHours;
	protected PermEstimatingManHoursCanPush = EstimatingManHours.PermEstimatingManHoursCanPush;
	protected PermCRMExportManHoursCSV = EstimatingManHours.PermCRMExportManHoursCSV;
	...
	protected AddDefinition(): void {
		Dialogues.Open({ name: 'ModifyManHourDialogue', state: null});
	}
