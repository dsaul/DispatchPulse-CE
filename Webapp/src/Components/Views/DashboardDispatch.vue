<template>
	...
	<div>
		...
		<v-app-bar color="#747389" dark fixed app clipped-right >
			...
			<v-toolbar-title class="white--text">Dispatch Dashboard</v-toolbar-title>
			...
			<template v-slot:extension>
				...
				<v-tabs
					...
					<v-tab
						...
						class="e2e-dashboard-tab-dispatch"
						:disabled="!PermCRMViewDashboardDispatchTab()"
						>
						All
					</v-tab>
		...
		<v-tabs-items v-model="tab" style="background: transparent;">
			...
			<v-tab-item style="flex: 1;">
				<div v-if="PermCRMViewDashboardDispatchTab()">
					<PastDueAssignmentsCard
						ref="pastDueAssignmentsCard"
						:disabled="connectionStatus != 'Connected'"
						/>
					<UnassignedAssignmentsCard
						ref="unassignedAssignmentsCard"
						:disabled="connectionStatus != 'Connected'"
						/>
					<DueAssignmentsWithNoLabourCard
						ref="dueAssignmentsWithNoLabourCard"
						:disabled="connectionStatus != 'Connected'"
						/>
				</div>
...
export default class Dashboard extends ViewBase {
	...
	public $refs!: {
		pastDueAssignmentsCard: PastDueAssignmentsCard,
		unassignedAssignmentsCard: UnassignedAssignmentsCard,
		dueAssignmentsWithNoLabourCard: DueAssignmentsWithNoLabourCard,
	...
	public ReLoadData(): void {
		...
		if (this.$refs.pastDueAssignmentsCard) {
			this.$refs.pastDueAssignmentsCard.LoadData();
		}
		if (this.$refs.unassignedAssignmentsCard) {
			this.$refs.unassignedAssignmentsCard.LoadData();
		}
		if (this.$refs.dueAssignmentsWithNoLabourCard) {
			this.$refs.dueAssignmentsWithNoLabourCard.LoadData();
		}
	}
	...
	public get IsLoadingData(): boolean {
		...
		if (this.$refs.pastDueAssignmentsCard && this.$refs.pastDueAssignmentsCard.IsLoadingData) {
			return true;
		}
		if (this.$refs.unassignedAssignmentsCard && this.$refs.unassignedAssignmentsCard.IsLoadingData) {
			return true;
		}
		if (this.$refs.dueAssignmentsWithNoLabourCard && this.$refs.dueAssignmentsWithNoLabourCard.IsLoadingData) {
			return true;
		}
